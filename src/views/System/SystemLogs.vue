<template>
  <div class="h-full flex flex-col bg-[#1e1e1e] text-gray-300 font-sans">
    <!-- Header -->
    <div class="flex-shrink-0 h-10 bg-[#252526] border-b border-[#3e3e42] flex items-center justify-between px-3 select-none">
       <div class="flex items-center gap-3">
           <button @click="router.back()" class="w-7 h-7 flex items-center justify-center rounded-full bg-gray-700 text-white hover:bg-gray-600 transition-colors">
       ...
       <div class="flex gap-2">
           <button @click="exportLogs" class="w-6 h-6 rounded hover:bg-white/10 flex items-center justify-center text-blue-400" title="导出日志">
               <i class="fa-solid fa-download text-xs"></i>
           </button>
           <button @click="clearLogs" class="w-6 h-6 rounded hover:bg-white/10 flex items-center justify-center text-red-400" title="清空日志">
    ...
    <div v-if="currentTab === 'logs'" class="flex-1 flex flex-col min-h-0">
        ...
        <div class="flex-shrink-0 p-2 bg-[#2d2d30] border-b border-[#3e3e42] flex gap-2 items-center">
            <select v-model="filterLevel" class="bg-[#3c3c3c] text-ccc border border-[#555] rounded px-2 py-0.5 text-xs outline-none">
            ...
            >
            <button 
               @click="loggerStore.autoScroll = !loggerStore.autoScroll"
        ...
        <!-- Log List -->
        <div ref="logsContainer" class="flex-1 overflow-y-auto p-2 font-mono text-xs space-y-2 custom-scrollbar bg-[#1e1e1e]">
            <div 
            ...
            >
               ...
               <div class="flex items-start gap-2">
                   ...
                   <!-- Badge Type -->
                   <span class="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider select-none min-w-[50px] text-center"
               ...
               <div v-if="log.detail" class="mt-1 ml-[70px]">
                   ...
                   >
                       <i class="fa-solid fa-play text-[8px] transition-transform" :class="{'rotate-90': isExpanded(log.id)}"></i>
                       [{{ isExpanded(log.id) ? '收起详情' : '查看详情 / Raw Data' }}]
                   </button>
                   
                   <!-- Detail Content -->
                   <div v-if="isExpanded(log.id)" class="mt-2 p-2 bg-black/30 rounded border border-gray-700/50 text-[10px] text-gray-400 overflow-x-auto whitespace-pre-wrap select-text">
    ...
    <div v-else class="flex-1 flex flex-col min-h-0 bg-[#1e1e1e]">
        <div class="p-2 border-b border-[#3e3e42] bg-[#2d2d30] flex justify-between items-center select-none">
            ...
            <button class="text-xs bg-green-700 text-white px-2 py-0.5 rounded hover:bg-green-600" @click="forceReview">
               刷新
            </button>
        </div>
        <div class="flex-1 overflow-y-auto p-4 text-xs font-mono text-gray-300 select-text whitespace-pre-wrap custom-scrollbar">
            <div v-if="contextData">
                 ...
                 <!-- Messages -->
                 <div v-for="(msg, idx) in contextMessages" :key="idx" class="mb-4 p-3 rounded border border-gray-700" :class="getRoleClass(msg.role)">
                     <div class="font-bold mb-2 uppercase flex justify-between items-center text-[10px] opacity-70">
                         <span><i class="fa-solid mr-1" :class="getRoleIcon(msg.role)"></i> [{{ idx }}] {{ msg.role }}</span>
...
const contextParams = computed(() => {
    ...
    const { model, temperature, max_tokens } = contextData.value
    return JSON.stringify({ model, temperature, max_tokens }, null, 2)
})

const contextMessages = computed(() => {
    return contextData.value?.messages || []
})
...
const getRoleClass = (role) => {
    ...
    if (role === 'assistant') return 'bg-blue-900/10 border-blue-900/30 text-blue-200'
