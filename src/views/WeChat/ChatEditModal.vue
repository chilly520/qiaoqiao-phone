  ...
  <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="close">
    <div class="bg-white rounded-2xl w-[90%] max-w-lg shadow-2xl flex flex-col max-h-[85vh] animate-scale-in">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/50" ref="listInfo" id="edit-msg-list">
        <!-- Dynamic Blocks -->
        <div v-for="(block, idx) in blocks" :key="idx" class="bg-white p-3 rounded-xl border border-gray-200 relative group shadow-sm hover:shadow-md transition-shadow">
           <!-- Remove Button -->
           <button v-if="blocks.length > 1" @click="removeBlock(idx)" class="absolute top-2 right-2 text-gray-300 hover:text-red-500 w-6 h-6 flex items-center justify-center rounded-full hover:bg-red-50 transition-colors z-10">
              <i class="fa-solid fa-xmark"></i>
           </button>
           ...
           <div class="flex gap-2 mb-2 pr-8">
              <!-- Role Select -->
              <select v-model="block.role" class="text-xs py-1 h-8 w-20 bg-gray-50 border border-gray-200 rounded px-2 focus:bg-white focus:ring-1 focus:ring-green-500 outline-none">
        ...
        <!-- Add Button -->
        <button @click="addBlock" class="w-full py-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-400 hover:border-green-500 hover:text-green-600 hover:bg-green-50 flex items-center justify-center gap-2 transition-all font-medium">
      ...
      <div class="p-4 border-t border-gray-100 flex gap-3 bg-white rounded-b-2xl">
        <button @click="close" class="flex-1 py-2.5 bg-gray-100 text-gray-600 rounded-lg font-medium hover:bg-gray-200 transition-colors">取消</button>
