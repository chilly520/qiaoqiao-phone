...
<template>
  <div class="chat-window w-full h-full flex flex-col overflow-hidden relative">
    ...
    <div class="h-[50px] bg-[#ededed] flex items-center justify-between px-3 border-b border-[#dcdcdc] shadow-sm z-10 relative">
        ...
        <div class="flex-1 flex flex-col items-center justify-center z-10 overflow-hidden cursor-pointer mx-[70px] h-full" @click="openStatusEditor">
            ...
            <div class="flex items-center gap-1.5 mt-0.5">
                <div class="w-2 h-2 rounded-full shadow-[0_0_4px_rgba(0,223,108,0.5)]" :class="chatData?.isOnline ? 'bg-[#00df6c]' : 'bg-gray-400'"></div>
                <span class="text-[10px] text-gray-500 truncate max-w-[150px] font-medium">{{ chatData?.statusText || '在线' }}</span>
            </div>
        </div>
        <div class="absolute right-1.5 flex items-center gap-0.5 text-black z-20">
             ...
             <div 
                ...
                @click="toggleAutoTTS"
                :title="chatData?.autoTTS ? '关闭朗读' : '开启朗读'"
             >
                <i class="fa-solid" :class="chatData?.autoTTS ? 'fa-volume-high text-green-600' : 'fa-volume-xmark text-gray-400'"></i>
             </div>

             <!-- Inner Voice Button -->
             <div 
                class="w-7 h-7 rounded-full flex items-center justify-center cursor-pointer transition-all hover:bg-black/5 relative"
                @click="openInnerVoiceModal"
                title="心声"
             >
    ...
    <!-- Messages Area -->
    <div 
        class="flex-1 overflow-y-auto p-4 flex flex-col gap-4 relative z-10" 
        ref="msgContainer"
        @click="closePanels"
    >
        ...
        <div v-if="showFriendRequest" class="w-full flex justify-center py-4 z-10">
             <div class="bg-blue-50/80 backdrop-blur-md border border-white/40 p-5 w-[90%] max-w-[340px] rounded-xl shadow-lg">
                 <div class="flex items-center gap-4 mb-4">
                     <img :src="chatData.avatar" class="w-14 h-14 rounded-lg bg-gray-200 object-cover border border-white/50 shadow-sm">
                     <div>
                 ...
                 <div class="flex gap-3">
                     <button class="flex-1 bg-white/60 hover:bg-white/80 text-gray-600 text-sm py-2.5 rounded-lg font-medium transition-colors border border-white/40" @click="handleIgnoreFriend">忽略</button>
                     <button class="flex-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm py-2.5 rounded-lg font-medium shadow-md active:scale-95 transition-transform" @click="handleAcceptFriend">同意</button>
                 </div>
             </div>
        </div>
        ...
        <div v-if="msgs.length === 0 && !showFriendRequest" class="text-center text-gray-400 text-xs py-4 flex flex-col items-center gap-2 z-10">
            <span>现在可以开始聊天了</span>
        ...
        <div 
          v-for="(msg, index) in displayedMsgs" 
          :key="msg.id" 
          v-show="isMsgVisible(msg)"
          class="w-full z-10"
        >
           <!-- Center Time Divider (WeChat Style) -->
           <div v-if="shouldShowTimeDivider(msg, displayedMsgs[index-1])" class="flex justify-center my-4 animate-fade-in relative">
           ...
           >
                ...
                <div class="flex-1 overflow-hidden" @click="isMultiSelectMode ? toggleMessageSelection(msg.id) : null">
           ...
           >
               ...
               >
                ...
                <div class="flex flex-col max-w-[70%]" :class="msg.role === 'user' ? 'items-end' : 'items-start'">
                    ...
                    >
                         <div class="pay-top">
                             <div class="pay-icon" :class="{'bg-[#ea5f39]': !msg.type || msg.type === 'redpacket', 'bg-[#eda338]': msg.type === 'transfer', 'opacity-70': msg.isClaimed}">
                                  <i v-if="msg.type === 'transfer'" :class="msg.isClaimed ? 'fa-solid fa-check' : 'fa-solid fa-right-left'"></i>
                                  <i v-else :class="msg.isClaimed ? 'fa-regular fa-envelope-open' : 'fa-regular fa-envelope'"></i>
                             </div>
                             <div class="pay-info">
                ...
                <div v-else-if="msg.type === 'voice'" class="flex flex-col w-full" :class="msg.role === 'user' ? 'items-end' : 'items-start'">
                    <div class="flex items-center gap-2" :class="msg.role === 'user' ? 'flex-row-reverse' : ''">
                         <!-- Bubble -->
                         <div 
                            class="h-10 rounded-lg flex items-center px-4 cursor-pointer relative shadow-sm min-w-[80px]"
                            :class="[
                         ...
                         >
                             <!-- Wave Icon -->
                             <div class="voice-wave" :class="[msg.isPlaying ? 'playing' : '', msg.role === 'user' ? 'wave-right' : 'wave-left']">
                                 <div class="bar bar1" :class="msg.role === 'user' ? 'bg-white' : 'bg-[#d4af37]'"></div>
                                 <div class="bar bar2" :class="msg.role === 'user' ? 'bg-white' : 'bg-[#d4af37]'"></div>
                                 <div class="bar bar3" :class="msg.role === 'user' ? 'bg-white' : 'bg-[#d4af37]'"></div>
                             </div>
                             ...
                             <!-- Duration (Inside) -->
                             <span class="text-[10px] font-bold opacity-70" :class="msg.role === 'user' ? 'mr-0 ml-1' : 'ml-1 mr-0'">{{ getDuration(msg) }}"</span>
                         </div>

                         <!-- Red Dot for Unplayed AI Voice -->
                         <div v-if="msg.role === 'ai' && !msg.isPlayed" class="w-2 h-2 bg-red-500 rounded-full"></div>
                    </div>
    ...
    <div v-if="!isMultiSelectMode" class="bg-[#f7f7f7] border-t border-[#dcdcdc] relative z-20" @contextmenu.prevent>
        
        <!-- Reply Bar Overlay -->
        <div v-if="currentQuote" class="absolute bottom-full left-0 right-0 mb-0 bg-white shadow-sm border-t border-gray-100 p-3 flex justify-between items-center z-30">
            <div class="text-sm text-gray-700 truncate max-w-[85%] border-l-4 border-gray-300 pl-2">
                <span class="font-medium text-gray-900">{{ currentQuote.role === 'user' ? '我' : (chatData.name || '对方') }}:</span> 
        ...
        <div class="flex items-center px-4 pt-2 pb-1 gap-4 text-[#4a4a4a] text-[18px] select-none overflow-x-auto no-scrollbar">
            <!-- Plus (Panel Toggle) -->
            <i class="fa-solid fa-circle-plus cursor-pointer hover:text-gray-800 transition-colors" @click="toggleActionPanel" title="更多功能"></i>

            <!-- Emoji -->
            <i class="fa-regular fa-face-smile cursor-pointer hover:text-gray-800 transition-colors" @click.stop="toggleEmojiPicker" title="表情"></i>
            ...
            <!-- Regenerate -->
            <i class="fa-solid fa-rotate-right cursor-pointer hover:text-blue-500 transition-colors" @click="regenerateLastMessage" title="重新生成"></i>
            
            <!-- Music -->
            <i class="fa-solid fa-music cursor-pointer hover:text-yellow-600 transition-colors" :class="{ 'text-yellow-500': musicStore.playerVisible }" @click="musicStore.togglePlayer" title="音乐 (一起听歌)"></i>
        </div>
        ...
        <div class="flex items-end px-3 pb-3 pt-1 gap-2">
             ...
             <div class="flex-1 bg-white rounded-lg border border-gray-300 flex items-center min-h-[38px] px-3 py-2 shadow-sm">
                 ...
                 <textarea 
                    ...
                    ref="textareaRef"
                    style="max-height: 66px; overflow-y: auto;"
                ></textarea>
             </div>
    ...
    <div v-if="isMultiSelectMode" class="h-[60px] bg-[#f7f7f7] border-t border-[#dcdcdc] flex items-center justify-between px-8 relative z-30 animate-fade-in-up">
        <button @click="exitMultiSelectMode" class="flex flex-col items-center justify-center text-gray-600 hover:text-gray-800 transition-colors">
            <i class="fa-solid fa-xmark text-lg"></i>
            <span class="text-[10px] mt-0.5">取消</span>
        </button>
        
        <div class="flex gap-12">
            <button @click="favoriteSelectedMessages" class="flex flex-col items-center justify-center text-gray-600 hover:text-[#07c160] transition-colors" :class="{'opacity-30': selectedMsgIds.size === 0}">
                <i class="fa-regular fa-star text-lg"></i>
                <span class="text-[10px] mt-0.5">收藏</span>
            </button>
            <button @click="deleteSelectedMessages" class="flex flex-col items-center justify-center text-red-500 hover:text-red-600 transition-colors" :class="{'opacity-30': selectedMsgIds.size === 0}">
    ...
    <ChatActionPanel 
       ...
       class="h-[200px] border-t border-[#dcdcdc] bg-[#f7f7f7] relative z-20"
  ...
  <!-- Red Packet Modal -->
  <div v-if="showRedPacketModal" class="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 animate-fade-in">
       <div class="w-[300px] h-[480px] bg-[#d95940] rounded-xl relative overflow-hidden shadow-2xl flex flex-col items-center pt-16">
           <button class="absolute top-2 left-2 text-white/80 p-2" @click="closeRedPacketModal">
               <i class="fa-solid fa-xmark text-xl"></i>
           </button>
           
           <!-- Top Curve Decoration -->
           <div class="absolute -top-[100px] -left-[50px] w-[400px] h-[300px] bg-[#e0664e] rounded-[50%] z-0"></div>
           
           <!-- Info -->
           <div class="z-10 flex flex-col items-center">
                <img :src="currentRedPacket?.role === 'user' ? 'https://api.dicebear.com/7.x/avataaars/svg?seed=Felix' : chatData?.avatar" class="w-16 h-16 rounded-full border-2 border-[#fcdba8] mb-3 shadow-md">
                <div class="text-[#fcdba8] text-lg font-medium">{{ currentRedPacket?.role === 'user' ? '我的红包' : chatData?.name + '的红包' }}</div>
                <div class="text-[#fcdba8]/80 text-sm mt-1">{{ currentRedPacket?.note || "恭喜发财，大吉大利" }}</div>
           </div>

           <!-- Open Button (Spinner) -->
           <div v-if="!showResult" class="flex flex-col items-center mt-20 z-10">
                <div class="w-24 h-24 bg-[#fcdba8] rounded-full flex items-center justify-center shadow-lg cursor-pointer transition-transform active:scale-95"
                    :class="{'animate-spin-slow': isOpening}"
                    @click="openRedPacket"
                >
                    <div class="text-[#d95940] font-bold text-2xl">開</div>
                </div>
                <!-- Reject Link (Only for Received Packets) -->
                <div v-if="currentRedPacket?.role === 'ai'" class="mt-4 text-[#fcdba8]/60 text-xs underline cursor-pointer" @click="rejectPayment">
                    拒收红包
                </div>
           </div>

           <!-- Result View -->
           <div v-else class="z-10 mt-12 flex flex-col items-center animate-scale-in">
                <!-- Status Text -->
                <div class="text-[#fcdba8] text-sm mb-2">
                    {{ 
                        currentRedPacket?.isRejected ? '已拒收' :
                        currentRedPacket?.role === 'user' ? (currentRedPacket?.isClaimed ? '红包已被领取' : '等待对方领取') : '已存入零钱' 
                    }}
                </div>
                
                <div class="text-[#fcdba8] text-4xl font-bold flex items-baseline">
                    <span class="text-2xl mr-1">¥</span>{{ resultAmount }}
                </div>
                <!-- If User Sent, show who received -->
                <div v-if="currentRedPacket?.role === 'user'" class="mt-8 flex flex-col items-center gap-2">
                    <div class="text-[#fcdba8]/80 text-xs">领取详情</div>
                    ...
                    <!-- Unclaimed State -->
                    <div v-else class="text-[#fcdba8]/60 text-sm py-2">
                        1个红包共{{ resultAmount }}元，暂未领取
                    </div>
                </div>
                <div class="text-[#fcdba8]/60 text-xs mt-8 cursor-pointer underline" @click="closeRedPacketModal">查看我的钱包</div>
           </div>
  ...
  <!-- Transfer Modal -->
  <div v-if="showTransferModal" class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
      <div class="bg-white w-[280px] rounded-lg overflow-hidden shadow-xl">
          <div class="p-6 flex flex-col items-center text-center">
              <div v-if="currentRedPacket?.isRejected">
                   <i class="fa-solid fa-ban text-red-500 text-4xl mb-4"></i>
                   <div class="text-lg font-medium text-black mb-1">已拒收</div>
                   <div class="text-2xl font-bold text-black mb-4">¥{{ currentRedPacket?.amount || '0.00' }}</div>
                   <div class="text-gray-500 text-sm mb-4">资金已退回对方账户</div>
              </div>
              <div v-else-if="currentRedPacket?.isClaimed">
    <i class="fa-solid fa-check-circle text-[#07c160] text-4xl mb-4"></i>
    <div class="text-lg font-medium text-black mb-1">已收款</div>
    <div class="text-2xl font-bold text-black mb-4">¥{{ currentRedPacket?.amount || '0.00' }}</div>
    <div class="text-gray-500 text-sm mb-4">已存入零钱</div>
</div>
<div v-else>
    <i class="fa-solid fa-circle-check text-[#07c160] text-4xl mb-4"></i>
    <div class="text-lg font-medium text-black mb-1">收到转账</div>
    <div class="text-2xl font-bold text-black mb-4">¥{{ currentRedPacket?.amount || '0.00' }}</div>
    <div class="text-gray-500 text-sm mb-4">确认收款后资金将存入零钱</div>

    <div class="flex flex-col gap-2 w-full">
        <button class="w-full bg-[#07c160] text-white py-2 rounded font-medium" @click="confirmTransfer">确认收款</button>
        <button class="w-full text-red-500 text-sm py-2" @click="rejectPayment">立即退还</button>
    </div>
</div>
           </div>
           <div class="bg-gray-50 p-3 text-center border-t border-gray-100 cursor-pointer" @click="showTransferModal = false">
              <span class="text-black font-medium">关闭</span>
          </div>
      </div>
  </div>

    <!-- Inner Voice Modal (Mindscape) -->
    <div v-if="showInnerVoiceModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" @click="closeInnerVoiceModal">
        <div class="voice-modal-content" @click.stop>
            ...
            <div class="voice-modal-header">
                ...
                <div class="flex gap-2">
                    <button class="voice-modal-header-btn" title="历史记录" @click="toggleVoiceHistory">
                        <i class="fa-solid" :class="showHistoryList ? 'fa-rotate-left' : 'fa-clock-rotate-left'"></i>
                    </button>
                    <button class="voice-modal-header-btn" title="删除当前" @click="deleteCurrent"><i class="fa-solid fa-trash-can" style="font-size: 14px;"></i></button>
                </div>
            </div>
            ...
            <div class="voice-modal-body" v-else>
                <div class="text-[#8c7e63] text-xs text-center mb-6 tracking-widest uppercase opacity-80">· Mindscape History ·</div>
                
                <div v-if="voiceHistoryList.length === 0" class="text-center text-[#8c7e63] mt-10 text-xs opacity-50">暂无历史记录</div>

                <div v-else class="voice-history-list">
                    <div v-for="(item, index) in voiceHistoryList" :key="item.id" 
                         class="voice-history-card animate-fade-in" 
    ...
    <!-- Send Money Modal -->
    <div v-if="showSendModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 animate-fade-in" @click.self="showSendModal = false">
        <div class="bg-white w-[85%] max-w-[320px] rounded-xl overflow-hidden shadow-2xl flex flex-col transition-all transform scale-100">
            <!-- Header -->
            <div class="bg-[#ededed] h-12 relative flex items-center justify-center border-b border-gray-200 shrink-0">
                <span class="font-bold text-gray-900 text-[17px]">{{ sendType === 'redpacket' ? '发红包' : '转账' }}</span>
                <div class="absolute right-0 top-0 h-full w-12 flex items-center justify-center cursor-pointer active:bg-gray-200 transition-colors" @click="showSendModal = false">
            ...
            <div class="p-6 flex flex-col gap-6">
                ...
                <div v-if="sendType === 'transfer'" class="flex flex-col items-center gap-2">
                    <img :src="chatData?.avatar" class="w-14 h-14 rounded-lg bg-gray-200 object-cover">
                    <div class="text-gray-900 text-sm">转账给 <span class="font-bold">{{ chatData?.name }}</span></div>
                </div>

                <!-- Amount Input -->
                <div class="flex flex-col gap-2">
                    <div class="text-gray-900 font-medium text-sm ml-1" v-if="sendType === 'transfer'">转账金额</div>
                    <div class="flex items-baseline gap-2 border-b-2 border-[#07c160]/50 focus-within:border-[#07c160] transition-colors pb-1 pt-2">
                         <span class="text-gray-900 font-bold text-3xl">¥</span>
                         <input type="number" 
                                v-model="sendAmount" 
                                placeholder="0.00" 
                                class="flex-1 bg-transparent border-none outline-none text-5xl font-bold text-gray-900 h-14 placeholder-gray-300" 
                                step="0.01"
                                autoFocus
                        >
                    </div>
                ...
                <!-- Note Input -->
                <div class="flex flex-col gap-2">
                     <label class="text-xs text-gray-500" v-if="sendType === 'redpacket'">寄语</label>
                     <label class="text-xs text-black font-medium" v-else>添加备注</label>
                     <input type="text" 
                            v-model="sendNote" 
                            :placeholder="sendType === 'redpacket' ? '恭喜发财，大吉大利' : '转账给您'" 
                            class="w-full bg-gray-50 rounded px-3 py-2 border-none text-sm outline-none placeholder-gray-400 focus:bg-white focus:ring-1 focus:ring-[#07c160] transition-all"
                    >
                </div>

                <button @click="confirmSend" 
                        class="w-full bg-[#07c160] text-white py-3.5 rounded-lg font-bold text-[17px] active:bg-[#06ad56] transition-transform active:scale-[0.98] shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
                        :disabled="!sendAmount"
                >
    ...
    <!-- Status Edit Modal -->
    <div v-if="showStatusModal" class="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-6 animate-fade-in" @click="showStatusModal = false">
            ...
            <div class="p-6">
                ...
                <div class="flex items-center justify-between mb-4 bg-gray-50 p-2 rounded-xl">
                    ...
                    <div class="flex bg-gray-200 rounded-lg p-1">
                        <button 
                            @click="statusIsOnline = true" 
                            class="px-3 py-1 rounded-md text-xs font-bold transition-all"
                            :class="statusIsOnline ? 'bg-[#00df6c] text-white shadow-sm' : 'text-gray-500'"
                        >在线</button>
                        <button 
                            @click="statusIsOnline = false" 
                            class="px-3 py-1 rounded-md text-xs font-bold transition-all"
                            :class="!statusIsOnline ? 'bg-gray-400 text-white shadow-sm' : 'text-gray-500'"
                        >离线</button>
                    </div>
                </div>
                <div class="flex gap-3">
                    <button @click="showStatusModal = false" class="flex-1 py-3 rounded-xl font-medium text-gray-500 bg-gray-100 active:bg-gray-200 transition-colors">取消</button>
                    <button @click="saveStatus" class="flex-1 py-3 rounded-xl font-medium text-white bg-[#07c160] active:bg-[#06ad56] shadow-sm transition-colors">确定</button>
                </div>
            </div>
        </div>
    </div>
    ...
    >
        <div class="fixed bg-[#2b2b2b] text-white rounded-lg shadow-xl py-2 w-[140px] border border-[#333] transition-opacity duration-200"
             :style="{ top: (menuPosition.y + 5) + 'px', left: (menuPosition.x + 5) + 'px' }"
             @click.stop
             @touchstart.stop
        >
            <div class="flex flex-col select-none">
                 <!-- Debug click -->
                 <div class="ctx-item" @click.stop="handleMenuAction('edit')" @touchstart.stop="handleMenuAction('edit')"><i class="fa-solid fa-pen w-5"></i> 编辑</div>
                 <div class="ctx-item" @click.stop="handleMenuAction('history')" @touchstart.stop="handleMenuAction('history')"><i class="fa-solid fa-clock-rotate-left w-5"></i> 编辑历史</div>
                 <div class="ctx-item" @click.stop="handleMenuAction('copy')" @touchstart.stop="handleMenuAction('copy')"><i class="fa-regular fa-copy w-5"></i> 复制</div>
                 <div class="ctx-item" @click.stop="handleMenuAction('quote')" @touchstart.stop="handleMenuAction('quote')"><i class="fa-solid fa-quote-left w-5"></i> 引用</div>
                 <div class="ctx-item" @click.stop="handleMenuAction('recall')" @touchstart.stop="handleMenuAction('recall')"><i class="fa-solid fa-rotate-left w-5"></i> 撤回</div>
                 <div class="ctx-item" @click.stop="handleMenuAction('fav')" @touchstart.stop="handleMenuAction('fav')"><i class="fa-regular fa-star w-5"></i> 收藏</div>
                 <div class="ctx-item" @click.stop="handleMenuAction('listen')" @touchstart.stop="handleMenuAction('listen')"><i class="fa-solid fa-volume-high w-5"></i> 听音</div>
                 <div class="ctx-divider my-1 border-t border-white/10"></div>
                 <div class="ctx-item" @click.stop="handleMenuAction('multi')" @touchstart.stop="handleMenuAction('multi')"><i class="fa-solid fa-list-check w-5"></i> 多选</div>
                 <div class="ctx-item text-red-400" @click.stop="handleMenuAction('delete')" @touchstart.stop="handleMenuAction('delete')"><i class="fa-solid fa-trash w-5"></i> 删除</div>
            </div>
        </div>
    </div>
  </div>
...
.chat-bubble-left :deep(p), .chat-bubble-right :deep(p) {
    margin: 0;
    line-height: 1.6;
}
.chat-bubble-left :deep(p + p), .chat-bubble-right :deep(p + p) {
    margin-top: 0.5em;
}
.chat-bubble-left :deep(ul), .chat-bubble-right :deep(ul) {
    list-style-type: disc;
    padding-left: 1.5em;
    margin: 0.5em 0;
}
.chat-bubble-left :deep(ol), .chat-bubble-right :deep(ol) {
    list-style-type: decimal;
    padding-left: 1.5em;
    margin: 0.5em 0;
}
.chat-bubble-left :deep(li), .chat-bubble-right :deep(li) {
    margin: 0.2em 0;
}
.chat-bubble-left :deep(code), .chat-bubble-right :deep(code) {
...
.chat-bubble-left :deep(pre), .chat-bubble-right :deep(pre) {
...
.chat-bubble-left :deep(pre code), .chat-bubble-right :deep(pre code) {
    background: transparent;
    padding: 0;
}
.chat-bubble-left :deep(a), .chat-bubble-right :deep(a) {
    color: #0066cc;
...
#voice-effect-canvas {
    ...
    pointer-events: none;
    z-index: 0;
    mix-blend-mode: screen; /* Crucial for the glowing effect */
}
...
.header-title {
    font-family: serif; /* Fallback since we might not have 'Cormorant Garamond' */
    font-size: 16px;
    letter-spacing: 6px;
    color: #e6dcc0;
...
.chat-bubble-left {
    ...
    color: #f0e6d2 !important; /* Champagne Gold Text */
...
.pay-card {
    width: 230px;
    background-color: #fa9d3b; /* Orange */
    border-radius: 4px;
    overflow: hidden;
    cursor: pointer;
    user-select: none;
...
.pay-card.received, .pay-card.rejected {
    background-color: #fab46e; /* Desaturated version of orange */
    opacity: 0.9;
}
...
#voice-effect-canvas {
    ...
    height: 100%;
    pointer-events: none;
    z-index: 1; /* Boosted from 0 */
    /* mix-blend-mode: screen; */ /* Temporarily disabled for visibility check */
}
...
.voice-wave .bar1 { height: 6px; }
.voice-wave .bar2 { height: 10px; }
.voice-wave .bar3 { height: 14px; }
