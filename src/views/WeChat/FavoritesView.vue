    ...
    <div class="h-full bg-gray-100 flex flex-col">
        ...
        <div v-if="showSearch" class="bg-white px-4 pb-2 -mt-1 border-b border-gray-100">
            <div class="bg-gray-100 rounded-lg flex items-center px-3 py-1.5">
                ...
                <input v-model="searchQuery" type="text" placeholder="搜索收藏内容..." class="bg-transparent border-none outline-none text-sm w-full text-gray-700 placeholder-gray-400">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3">
             ...
             <div v-for="item in favorites" :key="item.id" 
                  class="bg-white p-2 rounded-lg shadow-sm mb-3 flex flex-col"
             >
                 ...
                 <div class="active:bg-gray-50 transition-colors p-2 rounded cursor-pointer" @click="openDetail(item)">
                     <div class="flex items-center gap-2 mb-2">
                         ...
                         <div v-else class="bg-gray-100 w-8 h-8 rounded-full flex items-center justify-center text-xs text-gray-500 font-bold">
                             {{ item.author[0] }}
                         </div>
                     ...
                     <div class="text-sm text-gray-800 line-clamp-3">
                         ...
                         <template v-else-if="getPreviewContent(item).type === 'sticker_placeholder'">
                             <div class="inline-flex items-center gap-1 bg-gray-100 px-2 py-1 rounded text-gray-500 text-xs text-ellipsis overflow-hidden whitespace-nowrap max-w-full">
                 ...
                 <div class="mt-1 flex justify-end px-2 pb-1 relative z-10 pointer-events-auto">
                     <button class="px-4 py-2 bg-red-50 text-red-500 rounded active:bg-red-200 font-bold text-xs" @click.stop="deleteItem(item.id)">
                         删除
                     </button>
                 </div>
             </div>
