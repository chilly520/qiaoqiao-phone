...
<template>
  <div class="h-full bg-white flex flex-col animate-slide-in-right">
    ...
    <div class="flex-1 overflow-y-auto">
        <div v-if="momentsStore.notifications.length === 0" class="flex flex-col items-center justify-center h-40 text-gray-400">
            <span>暂无新消息</span>
        </div>

        <div 
            v-for="item in momentsStore.notifications" 
            :key="item.id"
            class="flex items-start px-4 py-3 active:bg-gray-50 transition-colors border-b border-gray-50"
        >
            <!-- Avatar -->
            ...
            <div class="w-14 h-14 bg-gray-100 ml-3 shrink-0 overflow-hidden" v-if="item.momentImage">
             ...
             <div class="w-14 h-14 bg-gray-100 ml-3 shrink-0 flex items-center justify-center text-xs text-gray-400" v-else>
                文章
            </div>
        </div>
    </div>
