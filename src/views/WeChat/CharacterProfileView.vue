<template>
  <div class="h-full flex flex-col bg-[#f2f2f2]">
    ...
    <div class="flex-1 overflow-y-auto no-scrollbar">
      ...
      <div class="relative w-full h-[280px] bg-gray-300" @click="showBackgroundModal = true">
        ...
        <div class="absolute -bottom-12 left-4 flex items-end gap-3">
          <div class="relative w-20 h-20">
            ...
            <div
              class="absolute overflow-hidden bg-white transition-all duration-300 rounded-lg border-4 border-white shadow-lg w-full h-full top-0 left-0">
      ...
      <div class="mt-14 px-4">
        ...
        <div
          ...
          @click="editBio" :class="!characterBio ? 'italic' : ''">
      ...
      <div v-if="pinnedMoments.length > 0" class="mt-6 px-4">
        ...
        <div class="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          <div v-for="moment in pinnedMoments" :key="moment.id"
            ...
            <img v-if="moment.images && moment.images[0]" :src="moment.images[0]" class="w-full h-full object-cover">
      ...
      <div class="mt-6">
        ...
        <div v-if="characterMoments.length > 0" class="space-y-0">
          <MomentItem v-for="moment in characterMoments" :key="moment.id" :moment="moment" />
        </div>
    ...
    <!-- Background Modal -->
    <div v-if="showBackgroundModal" class="fixed inset-0 z-[110] bg-black/60 flex items-center justify-center p-6"
        ...
        <div class="p-5 pt-8">
          ...
          <div class="space-y-4">
            <div>
              ...
              <button class="w-full mt-2 py-2 bg-blue-500 text-white rounded-lg font-bold text-sm active:bg-blue-600"
    ...
    <!-- Edit Bio Modal -->
    <div v-if="showBioModal" class="fixed inset-0 z-[120] bg-black/60 flex items-center justify-center p-6"
      @click.self="showBioModal = false">
      <div class="bg-white w-full max-w-[340px] rounded-2xl overflow-hidden shadow-2xl animate-scale-up">
        <div class="p-5 pt-8">
          ...
          <textarea v-model="bioInput"
            ...
            placeholder="写下你的个性签名..." maxlength="100"></textarea>

          <div class="text-xs text-gray-400 text-right mt-1">{{ bioInput.length }}/100</div>

          <div class="flex gap-2 mt-4">
            <button class="flex-1 py-3 rounded-xl bg-gray-100 text-gray-600 font-bold active:bg-gray-200"
              @click="showBioModal = false">
              取消
            </button>
            <button class="flex-1 py-3 rounded-xl bg-blue-500 text-white font-bold active:bg-blue-600" @click="saveBio">
              保存
            </button>
          </div>
        </div>
...
const generateProfile = async () => {
  ...
  try {
    ...
    const userProfile = { name: settingsStore.personalization.userProfile.name }
    const { pinnedMoments: pinned, backgroundUrl, bio } = await generateCompleteProfile(character.value, userProfile)
