...
// Flatten all entries from all books
const allWorldBookEntries = computed(() => {
    const entries = []
    stickerStore.worldBookSync?.books?.forEach(book => {
    ...
    // If stickStore sync is not set, use worldBookStore directly
    if (entries.length === 0) {
        momentsStore.worldBookStore?.books?.forEach(book => {
            book.entries?.forEach(entry => {
                entries.push({
                    ...entry,
                    bookName: book.name
...
const filteredMoments = computed(() => {
    if (!momentsStore?.sortedMoments) return []
    if (!filterAuthorId.value) return momentsStore.sortedMoments
    return momentsStore.sortedMoments.filter(m => m.authorId === filterAuthorId.value)
})
...
const handlePost = () => {
    ...
    // Reset form
    postForm.value = { content: '', images: [], stickers: [], imageDescription: '' }
...
const confirmClearCharId = ref(null) // ID of char pending deletion
...
const triggerBatchGenerate = async () => {
    ...
    try {
        await momentsStore.batchGenerateAIMoments(Math.floor(Math.random() * 4) + 2) // 2-5 posts
...
const handleClearCharacterMoments = (charId) => {
    if (confirmClearCharId.value === charId) {
        // Confirmed
        const chat = chatStore.chats[charId] || { name: '该角色' }
        momentsStore.clearCharacterMoments(charId)
        chatStore.triggerToast(`已清空 ${chat.name} 的动态`, 'info')
...
<template>
  <div class="moments-view w-full h-full bg-[#ededed] flex flex-col relative overflow-hidden">
    <!-- Notifications Page (Overlay) -->
    <div v-if="showNotifications" class="absolute inset-0 z-50 bg-white">
        <MomentsNotifications @back="showNotifications = false" />
    </div>

    <!-- Header -->
    <div class="h-[72px] pt-7 shrink-0 flex items-center justify-between px-4 bg-transparent absolute top-0 left-0 right-0 z-20 text-white transition-colors duration-300 overflow-hidden" :class="{'bg-[#ededed] !text-black': filterAuthorId}">
        <i class="fa-solid fa-chevron-left text-xl cursor-pointer drop-shadow-md" @click="filterAuthorId ? clearFilter() : (showingProfileCharId ? handleProfileBack() : goBack())"></i>
        <div v-if="filterAuthorId" class="flex-1 text-center font-bold text-lg truncate px-8">{{ chatStore.chats[filterAuthorId]?.name }}的相册</div>
        ...
        <div class="flex gap-5 items-center">
            ...
            <div v-if="!filterAuthorId" class="relative cursor-pointer" @click="showNotifications = true">
                 ...
                 <div v-if="momentsStore.unreadCount > 0" class="absolute -top-1 -right-2 min-w-[16px] h-4 bg-red-500 text-white text-[10px] flex items-center justify-center rounded-full px-1 border border-white">
            ...
            <i v-if="!filterAuthorId" class="fa-solid fa-gear text-xl cursor-pointer drop-shadow-md" @click="openSettings"></i>
            <i v-if="!filterAuthorId" class="fa-solid fa-camera text-xl cursor-pointer drop-shadow-md" @click="showPostModal = true"></i>
        </div>
    </div>
    ...
    <div ref="scrollContainer" class="flex-1 overflow-y-auto no-scrollbar">
        <!-- Hero Section -->
        <div class="relative w-full h-[280px] bg-gray-300 mb-2 cursor-pointer" @click="openBackgroundModal">
            <img :src="backgroundUrl" 
                 class="w-full h-full object-cover">
            
            <!-- User Profile Link -->
            <div class="absolute -bottom-6 right-4 flex items-center gap-3">
                <span class="text-white font-bold text-lg drop-shadow-md mb-2">{{ userProfile.name }}</span>
                <div class="w-16 h-16 rounded-lg overflow-hidden border-2 border-white shadow-lg bg-white">
        ...
        <!-- Moments Feed -->
        <div class="px-0 pb-10 pt-6" :class="{'pt-[72px]': filterAuthorId}">
            <div v-if="filteredMoments.length === 0" class="flex flex-col items-center justify-center py-20 opacity-30">
            ...
            <MomentItem 
                ...
                @show-profile="id => showingProfileCharId = id"
            />
        </div>
    ...
    <div v-if="showPostModal" class="fixed inset-0 z-[100] bg-white animate-slide-up flex flex-col">
        <div class="h-[72px] pt-7 flex items-center justify-between px-4 border-b border-gray-100 shrink-0">
            ...
            <button 
                class="bg-[#07c160] text-white px-4 py-1.5 rounded text-sm font-bold disabled:opacity-50"
                @click="handlePost"
                :disabled="!postForm.content && postForm.images.length === 0"
            >
                {{ editingId ? '保存' : '发表' }}
            </button>
        </div>
        <div class="p-6 flex-1 overflow-y-auto">
            ...
            <div class="flex flex-wrap gap-2 mt-4">
                <!-- Images -->
                <div v-for="(img, idx) in postForm.images" :key="'img-'+idx" class="w-20 h-20 bg-gray-100 rounded relative group">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity rounded">
                        <i class="fa-solid fa-trash text-white cursor-pointer" @click="postForm.images.splice(idx, 1)"></i>
                    </div>
                </div>

                <!-- Stickers -->
                <div v-for="(sticker, idx) in postForm.stickers" :key="'stk-'+idx" class="w-20 h-20 bg-gray-100 rounded relative group">
                    <img :src="sticker" class="w-full h-full object-contain rounded p-1">
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity rounded">
                        <i class="fa-solid fa-trash text-white cursor-pointer" @click="postForm.stickers.splice(idx, 1)"></i>
                    </div>
                </div>

                <!-- Add Button -->
                <div 
                    v-if="postForm.images.length + postForm.stickers.length < 9"
            ...
            <div class="flex gap-4 mt-6 text-xl text-gray-500">
                <i class="fa-regular fa-face-smile cursor-pointer hover:text-gray-800" @click="showEmojiPicker = true"></i>
                <i class="fa-solid fa-link cursor-pointer hover:text-gray-800" @click="postForm.images.push(prompt('请输入图片URL'))"></i>
            </div>

            <input type="file" ref="fileInput" class="hidden" accept="image/*" multiple @change="handleLocalImageUpload">

            <div class="mt-8 border-t border-gray-100 pt-4">
                <div class="flex items-center gap-3 text-sm text-gray-500 mb-2">
                    <i class="fa-solid fa-eye text-blue-400"></i>
                    <span>AI 视觉描述（帮助 AI 看懂你的图）</span>
                </div>
                <input 
                    v-model="postForm.imageDescription"
                    type="text" 
                    placeholder="例如：一张我排队买奶茶的照片"
                    class="w-full bg-gray-50 px-3 py-2 rounded outline-none text-sm"
                >
            </div>
        </div>
    </div>
    ...
    <!-- Settings Modal -->
    <div v-if="showSettingsModal" class="fixed inset-0 z-[100] bg-black/60 flex items-center justify-center p-6" @click.self="showSettingsModal = false">
        <div class="bg-white w-full max-w-[340px] rounded-2xl overflow-hidden shadow-2xl animate-scale-up">
            <div class="p-5 pt-8">
                ...
                <div class="text-center text-[10px] text-blue-500 bg-blue-50 py-1.5 px-3 rounded-full inline-block mx-auto mb-6 flex items-center justify-center gap-1.5 border border-blue-100">
                ...
                <div class="space-y-6 max-h-[60vh] overflow-y-auto pr-1 custom-scrollbar">
                    <!-- Interval -->
                    <div>
                        <label class="text-xs text-gray-400 block mb-2 font-bold uppercase tracking-wider">自动生成频率 (分钟)</label>
                        <select v-model="settingsForm.autoGenerateInterval" class="w-full bg-gray-50 px-4 py-3 rounded-xl outline-none border border-gray-100">
                            <option :value="0">关闭自动生成</option>
                    ...
                    <!-- Characters -->
                    <div>
                        <label class="text-xs text-gray-400 block mb-2 font-bold uppercase tracking-wider">活跃角色筛选</label>
                        <div class="max-h-[150px] overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                            ...
                            >
                                <input type="checkbox" :checked="settingsForm.enabledCharacters.length === 0 || settingsForm.enabledCharacters.includes(chat.id)" class="accent-green-500">
                                ...
                                <i class="fa-solid transition-all p-1 active:scale-75 cursor-pointer" 
                                   ...
                                   :title="confirmClearCharId === chat.id ? '点击确认清空' : '清空此角色动态'"
                                ></i>
                            </div>
                        </div>
                    ...
                    <div>
                        <label class="text-xs text-gray-400 block mb-2 font-bold uppercase tracking-wider">世界书内容联动 (手动勾选)</label>
                        <div v-if="allWorldBookEntries.length > 0" class="max-h-[150px] overflow-y-auto space-y-2 pr-2 custom-scrollbar bg-blue-50/30 p-2 rounded-xl border border-blue-100/50">
                            <div 
                                v-for="entry in allWorldBookEntries" 
                                :key="entry.id"
                            ...
                            >
                                <input type="checkbox" :checked="settingsForm.enabledWorldBookEntries.includes(entry.id)" class="accent-blue-500">
                                <div class="flex-1">
                                    <div class="text-sm font-medium text-gray-700">{{ entry.name }}</div>
                                    <div class="text-[10px] text-gray-400 truncate w-40">{{ entry.bookName }} | {{ entry.content }}</div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Prompt -->
                    <div>
                        <label class="text-xs text-gray-400 block mb-2 font-bold uppercase tracking-wider">朋友圈自定义提示词</label>
                        <textarea 
                            v-model="settingsForm.customPrompt"
                            placeholder="例如：让他们多发一些关于学习和生活的动态"
                            class="w-full bg-gray-50 px-4 py-3 rounded-xl outline-none h-20 text-sm border border-gray-100"
                        ></textarea>
                    </div>
                </div>
                ...
                <div class="flex gap-3 mt-8">
                    <button 
                        class="flex-1 py-3 rounded-xl bg-red-50 text-red-500 font-bold active:bg-red-100 transition-colors" 
                        :class="clearConfirmMode ? 'bg-red-500 text-white shadow-lg shadow-red-200' : ''"
                        @click="handleClearAll"
                    >
                        {{ clearConfirmMode ? '再次点击确认清空' : '清空朋友圈' }}
                    </button>
                    <button class="flex-1 py-3 rounded-xl bg-[#07c160] text-white font-bold active:bg-[#06ad56] shadow-md shadow-green-100" @click="saveSettings">保存配置</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div v-if="showEmojiPicker" class="absolute inset-0 z-[110] flex items-end justify-center bg-black/20" @click="showEmojiPicker = false">
    ...
    <div v-if="showBackgroundModal" class="absolute inset-0 z-[110] bg-black/60 flex items-center justify-center p-6" @click.self="showBackgroundModal = false">
        <div class="bg-white w-full max-w-[340px] rounded-2xl overflow-hidden shadow-2xl animate-scale-up">
            <div class="p-5 pt-8">
    ...
    <div v-if="showingProfileCharId" class="absolute inset-0 z-[120] bg-[#f2f2f2] animate-fade-in flex flex-col">
        <div class="h-[72px] pt-7 flex items-center px-4 absolute top-0 left-0 right-0 z-10">
            <i class="fa-solid fa-chevron-left text-xl text-white drop-shadow-md cursor-pointer" @click="handleProfileBack"></i>
        </div>
        ...
        <div class="flex-1 overflow-y-auto bg-[#f2f2f2]">
                ...
                <div class="absolute bottom-10 left-6 right-6 text-white">
                    <div class="flex items-end justify-between mb-2">
                        <div>
                            ...
                            <p class="text-xs opacity-70 tracking-wider">微信号：{{ currentProfileChar.wechatId || 'wxid_unknown' }}</p>
                        </div>
            ...
            <div class="px-4 -mt-6 relative z-10">
                <div class="bg-white rounded-2xl shadow-sm p-4 space-y-4">
                    ...
                    >
                        ...
                        <i class="fa-solid fa-chevron-right text-gray-300 text-sm group-hover:translate-x-1 transition-transform"></i>
                    </button>
            ...
            <div class="p-6 pt-8">
                <div class="mb-6">
                    <label class="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-3">个性签名 / 设定</label>
                ...
                <div v-if="currentProfileChar.tags && currentProfileChar.tags.length > 0" class="flex flex-wrap gap-2">
                    <span v-for="tag in currentProfileChar.tags" :key="tag" class="text-[10px] bg-white px-2 py-0.5 rounded text-gray-400 border border-gray-100">
                        #{{ tag }}
                    </span>
                </div>
            </div>
