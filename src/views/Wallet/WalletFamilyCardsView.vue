  ...
  <div class="h-full flex flex-col transition-colors duration-300 font-sans"
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4 space-y-6">
        ...
        <div v-for="card in walletStore.familyCards" :key="card.id" 
             ...
             :class="getThemeClass(card.theme)"
             ...
             <div class="relative z-10 flex flex-col h-full justify-between">
                 ...
                 <div class="flex items-center gap-4">
                     <div class="w-12 h-12 rounded-2xl border-2 border-[#d4af37]/30 overflow-hidden bg-gray-900 shadow-xl p-0.5">
                         <img :src="getCharAvatar(card.ownerId)" class="w-full h-full object-cover rounded-[10px]">
                     </div>
                     <div>
                         <div class="font-bold text-[18px] leading-tight mb-1 tracking-tight text-shadow-gold">{{ card.remark || '亲属卡' }}</div>
                 ...
                 <div class="flex justify-between items-end border-t border-[#d4af37]/10 pt-4">
                     <div>
                         ...
                         <div class="text-3xl font-bold font-mono tracking-tighter text-shadow-gold">¥ {{ (card.amount - card.usedAmount).toFixed(2) }}</div>
                     </div>
                     <div class="text-right">
                         <div class="text-[9px] font-mono tracking-widest opacity-30 mb-1 rotate-0 origin-right">MEMBER ID: {{ card.id.substring(0,6) }}</div>
    ...
    <div v-if="showApplyModal" class="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-end sm:items-center sm:justify-center animate-fade-in" @click.self="showApplyModal = false">
        <div class="w-full sm:w-[380px] h-[75vh] sm:h-[550px] rounded-t-[32px] sm:rounded-[32px] flex flex-col animate-slide-up shadow-2xl transition-colors duration-300"
            ...
            <div class="p-6 border-b transition-colors flex items-center justify-between"
                ...
                <h3 class="font-bold text-xl tracking-tight" :class="settingsStore.personalization.theme === 'dark' ? 'text-white' : 'text-gray-900'">选择赠送方</h3>
                <button @click="showApplyModal = false" class="w-8 h-8 flex items-center justify-center rounded-full transition-colors"
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-2">
                <div v-for="char in availableCharacters" :key="char.id"
                     ...
                     <div class="flex-1 min-w-0">
                         <div class="font-bold text-base truncate" :class="settingsStore.personalization.theme === 'dark' ? 'text-gray-100' : 'text-gray-900'">{{ char.remark || char.name }}</div>
                         <div class="text-[11px] mt-1 line-clamp-1 opacity-60" :class="settingsStore.personalization.theme === 'dark' ? 'text-gray-400' : 'text-gray-500'">{{ char.statusText || 'Ta很神秘，什么都没写' }}</div>
                     </div>
                ...
                <div v-if="availableCharacters.length === 0" class="text-center py-20 text-gray-400 opacity-50">暂无可申请的对象</div>
            </div>
        </div>
    </div>

    <!-- Apply Modal Step 2: Input Remark & Amount -->
    <div v-if="showRemarkModal" class="fixed inset-0 z-[60] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in" @click.self="showRemarkModal = false">
        <div class="w-full max-w-[340px] rounded-[32px] overflow-hidden shadow-gold/20 shadow-2xl animate-scale-up border border-[#d4af37]/30 bg-[#1c1c1e]">
            <div class="bg-gradient-to-br from-[#2c2c2e] to-[#141414] p-8 text-center border-b border-[#d4af37]/20 relative overflow-hidden">
                <div class="absolute -right-10 -top-10 w-32 h-32 bg-[#d4af37]/5 rounded-full blur-3xl"></div>
                <div class="w-20 h-20 bg-gradient-to-br from-[#d4af37] via-[#f9e59d] to-[#b8860b] rounded-[24px] mx-auto mb-4 flex items-center justify-center text-gray-900 shadow-[0_10px_30px_rgba(212,175,55,0.4)] transform -rotate-[10deg]">
            ...
            <div class="p-8 space-y-6">
                <div class="flex items-center gap-4 bg-white/5 p-3 rounded-2xl border border-white/5">
                    <img :src="applyingTarget?.avatar" class="w-12 h-12 rounded-xl object-cover border border-[#d4af37]/20 shadow-lg">
                    <div class="flex-1">
                        <div class="text-[10px] text-gray-500 font-bold uppercase tracking-wider mb-0.5">APPLICANT TARGET</div>
                        <div class="text-[#e6dcc0] font-bold text-base">{{ applyingTarget?.remark || applyingTarget?.name }}</div>
                    </div>
                </div>

                <div class="space-y-2">
                    <label class="text-[11px] text-[#d4af37]/60 font-bold uppercase tracking-widest ml-1">申请留言 / REMARK</label>
                ...
                <div class="flex gap-4 pt-2">
                    <button class="flex-1 py-4 rounded-2xl bg-white/5 text-gray-400 font-bold text-sm active:scale-95 transition-all border border-white/5"
                        @click="showRemarkModal = false">
                        取消
                    </button>
                    <button class="flex-[1.5] py-4 rounded-2xl bg-gradient-to-r from-[#d4af37] to-[#b8860b] text-[#1c1c1e] font-bold text-sm shadow-xl shadow-[#d4af37]/20 active:scale-95 transition-all border border-white/20"
                        @click="confirmApply">
                        发送请求
                    </button>
                </div>
...
<style scoped>
.text-shadow-gold { text-shadow: 0 2px 8px rgba(212, 175, 55, 0.4); }
