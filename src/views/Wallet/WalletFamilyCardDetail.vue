<template>
  <div class="h-full flex flex-col transition-colors duration-300 font-sans"
    ...
    <div class="p-6 transition-colors"
      ...
      <div
        class="relative w-full aspect-[1.58] rounded-[24px] p-8 text-[#e6dcc0] shadow-2xl overflow-hidden border border-[#d4af37]/30 group"
        :class="getThemeClass(cardDetail?.theme)">
        <!-- Decor -->
        <div
          class="absolute -right-8 -top-8 w-48 h-48 bg-[#d4af37]/10 rounded-full blur-3xl group-hover:bg-[#d4af37]/20 transition-all duration-700">
        </div>
        ...
        <div class="relative z-10 flex flex-col h-full justify-between">
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              <div class="font-bold text-[20px] leading-tight mb-1 tracking-tight text-shadow-gold">{{
                cardDetail?.remark || '亲属卡' }}</div>
    ...
    <div class="flex-1 rounded-t-[32px] overflow-hidden flex flex-col shadow-inner transition-colors border-t"
      ...
      <div class="px-6 py-5 border-b transition-colors"
        ...
        <h3 class="font-bold text-base tracking-tight"
          :class="settingsStore.personalization.theme === 'dark' ? 'text-gray-200' : 'text-gray-800'">消费明细</h3>
      </div>
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <div v-else class="divide-y transition-colors"
          ...
          <div v-for="tx in cardDetail?.transactions" :key="tx.id"
            ...
            <div class="flex items-center gap-4">
              ...
              <div>
                <div class="font-bold text-[15px] mb-1"
                  :class="settingsStore.personalization.theme === 'dark' ? 'text-gray-100' : 'text-gray-800'">
                  {{ tx.title || '消费' }}
                </div>
...
const getCharAvatar = (id) => {
  ...
  // Fallback: Helper to find by name if ID lookup fails
  if (cardDetail.value?.ownerName) {
    const chat = Object.values(chatStore.chats).find(c => c.name === cardDetail.value.ownerName)
