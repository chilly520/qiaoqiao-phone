<template>
  <div class="h-full flex flex-col bg-[#ededed]">
    ...
    <div class="h-[60px] bg-[#ededed] flex items-center px-4 relative shrink-0">
      ...
      <div class="absolute right-4 w-10 h-10 flex items-center justify-center cursor-pointer active:bg-black/5 rounded-full" @click="showAddCard = true">
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        <div v-if="walletStore.bankCards.length === 0" class="flex flex-col items-center justify-center py-20 text-gray-400">
        ...
        <div v-for="card in walletStore.bankCards" :key="card.id" 
             ...
             :class="getCardThemeClass(card.theme)"
            ...
            <div class="relative z-10 flex flex-col h-full justify-between">
                ...
                <div class="space-y-1">
                    <div class="text-2xl font-mono tracking-widest text-shadow">{{ formatCardNumber(card.number) }}</div>
    ...
    <!-- Add Card Modal -->
    <div v-if="showAddCard" class="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center sm:justify-center p-4 animate-fade-in" @click.self="showAddCard = false">
        <div class="bg-white w-full max-w-sm rounded-[24px] overflow-hidden shadow-2xl animate-scale-up">
            <div class="p-6">
                ...
                <div class="space-y-4">
                    <!-- Bank Name -->
                    <div class="bg-gray-50 p-3 rounded-xl border border-gray-100 focus-within:border-blue-500 focus-within:bg-white transition-colors">
                        <label class="text-xs text-gray-400 font-bold ml-1 block mb-1">银行名称</label>
                        <input v-model="newCard.bankName" type="text" placeholder="例如：中国工商银行" class="w-full bg-transparent outline-none text-gray-800 font-medium">
                    </div>

                    <!-- Balance -->
                    <div class="bg-gray-50 p-3 rounded-xl border border-gray-100 focus-within:border-blue-500 focus-within:bg-white transition-colors">
                        <label class="text-xs text-gray-400 font-bold ml-1 block mb-1">初始余额</label>
                        <input v-model="newCard.balance" type="number" placeholder="0.00" class="w-full bg-transparent outline-none text-gray-800 font-medium">
                    </div>

                    <!-- Card Number -->
                    <div class="bg-gray-50 p-3 rounded-xl border border-gray-100 focus-within:border-blue-500 focus-within:bg-white transition-colors">
                        <label class="text-xs text-gray-400 font-bold ml-1 block mb-1">卡号 (选填，留空自动生成)</label>
                        <input v-model="newCard.number" type="text" placeholder="xxxx xxxx xxxx xxxx" class="w-full bg-transparent outline-none text-gray-800 font-bold font-mono">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex justify-between gap-2">
                             <div v-for="theme in themes" :key="theme.value" 
                                  @click="newCard.theme = theme.value"
...
const viewCardDetail = (card) => {
    // Show simple detail (can be expanded to a new view later)
    // For now, let's just toast
    // router.push({ name: 'card-detail', params: { id: card.id } })
    chatStore.triggerToast(`已选中: ${card.bankName}`, 'info')
}
</script>
