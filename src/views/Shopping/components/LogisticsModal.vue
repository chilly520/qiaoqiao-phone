...
const courierName = computed(() => {
    // [BUG FIX] trackingNumber 末尾可能非数字 (如 "SF123XYZ"), parseInt 返回 NaN,
    // 导致 courierNames[NaN] 为 undefined. 需 NaN 保护.
    const seed = parseInt(props.logistics.trackingNumber?.slice(-3) || '0')
    return courierNames[(isNaN(seed) ? 0 : seed) % courierNames.length]
})

const courierPhone = computed(() => {
    const seed = parseInt(props.logistics.trackingNumber?.slice(-2) || '0')
    return courierPhones[(isNaN(seed) ? 0 : seed) % courierPhones.length]
})
</script>
