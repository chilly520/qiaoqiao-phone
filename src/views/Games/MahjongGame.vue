<template>
    <div @click="unlockAudio"
        ...
        <div class="w-full h-full max-w-[600px] max-h-[1200px] relative flex flex-col shadow-2xl overflow-hidden">
            ...
            <!-- 顶部信息栏 -->
            <div
                class="game-top-bar h-[44px] shrink-0 bg-black/40 flex items-center justify-between px-4 z-50 relative border-b border-white/5">
                <div class="flex items-center gap-3">
                    <button @click="handleExit" title="结束对局"
                        class="w-8 h-8 flex items-center justify-center text-white/80 hover:text-red-400 active:scale-90 transition-all bg-white/5 rounded-full">
                        <i class="fa-solid fa-xmark text-lg"></i>
                    </button>
                    <button @click="handleMinimize" title="最小化挂机"
                        class="w-8 h-8 flex items-center justify-center text-white/80 hover:text-blue-400 active:scale-90 transition-all bg-white/5 rounded-full">
                        <i class="fa-solid fa-minus text-lg"></i>
                    </button>
                </div>

                <div class="flex items-center gap-6 text-white text-sm font-bold">
                    <span class="opacity-80">局数: <span class="text-white">{{ mahjongStore.currentRoom?.currentRound
                            }}/{{
                                mahjongStore.currentRoom?.totalRounds }}</span></span>
                    <span class="opacity-80">底注: <span class="text-white">{{ mahjongStore.currentRoom?.baseStake
                            }}</span></span>
                    <span class="opacity-80">牌堆: <span class="text-yellow-400">{{ mahjongStore.gameState?.deck?.length
                        || 0
                            }}</span></span>
                </div>
                ...
                <div class="flex items-center gap-1">
                    <button v-if="mahjongStore.currentRoom?.mode !== 'quick'"
                        ...
                        class="w-10 h-10 rounded-full bg-blue-500/80 text-white flex items-center justify-center active:scale-95 transition-transform mr-1 relative">
                        <i class="fa-solid fa-message text-lg"></i>
                    </button>
                    <button @click="showCardCounter = !showCardCounter"
                        class="w-10 h-10 rounded-full bg-white/10 text-white flex items-center justify-center active:scale-95 transition-transform mr-1">
                        <i class="fa-solid fa-calculator text-lg"></i>
                    </button>
                    <button @click="mahjongStore.cheatMode = !mahjongStore.cheatMode"
                        class="w-8 h-8 flex items-center justify-center text-white opacity-60 hover:opacity-100">
            ...
            <div class="absolute inset-0 pointer-events-none z-[80] overflow-hidden">
                <div v-for="dm in danmakuList" :key="dm.id"
                    ...
                    <!-- 头像 (无背景框，仅保留头像本身) -->
                    <div
                        class="w-10 h-10 rounded-full overflow-hidden border border-white/50 shadow-[0_2px_4px_rgba(0,0,0,0.6)] shrink-0 bg-black/20 flex items-center justify-center">
                        <img v-if="isImageAvatar(dm.avatar)" :src="dm.avatar" class="w-full h-full object-cover" />
                        <div v-else class="text-sm font-bold text-white drop-shadow-md">
                            {{ dm.avatar || (dm.sender && dm.sender[0]) || '👤' }}
                        </div>
                    </div>

                    <!-- 文字部分 (使用强投影代替背景盒) -->
                    <div class="flex items-center gap-1.5 whitespace-nowrap px-1"
                        style="text-shadow: 1px 1px 0 #000, -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 4px rgba(0,0,0,0.8);">
                        <span class="text-lg font-black tracking-wide" :style="{ color: dm.color }">
                            {{ dm.sender }}:
                        </span>
                ...
                <div class="flex-1 flex justify-between items-center w-full px-1 min-h-[350px]">
                    ...
                    <div class="flex-1 flex flex-col items-center justify-center min-w-0 mx-2">
                        ...
                        <div v-else class="flex flex-col items-center w-full scale-90 md:scale-100">
                            ...
                            <!-- 牌池（打出的牌） - 缩小 Tray 背景 -->
                            <div
                                class="bg-black/30 p-2 rounded-xl border border-white/10 shadow-inner w-full max-w-[220px] min-h-[80px] flex flex-wrap gap-1 justify-center relative overflow-visible">
                                <!-- 缩小刚打出的牌提示 -->
                ...
                <div v-if="mahjongStore.gameState?.roundResult"
                    ...
                    <Transition name="zoom">
                        <div v-if="showScoreCard"
                            class="bg-gradient-to-br from-amber-50 to-orange-100 w-full max-w-[min(90vw,440px)] rounded-[32px] overflow-hidden shadow-2xl border-4 border-amber-400 flex flex-col max-h-[85vh]"
                            :class="{ '!border-gray-400 !from-gray-100 !to-gray-200': mahjongStore.gameState.roundResult.type === '流局' }"
                            @click.stop>
                            <!-- 背景装饰 - 缩小高度 -->
                            <div class="relative p-4 md:p-6 text-center transition-all bg-gradient-to-r flex-shrink-0"
                                :class="mahjongStore.gameState.roundResult.type === '流局' ? 'from-gray-600 to-gray-700' : 'from-red-600 to-orange-600'">
                                <div class="text-4xl md:text-5xl mb-1 animate-bounce">
                                    {{ mahjongStore.gameState.roundResult.type === '流局' ? '💨' : '🎉' }}
                                </div>
                                <h2 class="text-xl md:text-2xl font-bold text-white mb-0.5 opacity-90">
                                    {{ mahjongStore.gameState.roundResult.winner.name }}
                                </h2>
                                <div class="text-3xl md:text-4xl font-black drop-shadow-md"
                                ...
                                <div v-if="mahjongStore.gameState.roundResult.type !== '流局'"
                                    class="mt-1.5 inline-block px-2.5 py-0.5 bg-white/20 rounded-full text-white text-[10px] backdrop-blur-sm">
                                    {{ mahjongStore.gameState.roundResult.isZiMo ? '自摸' : '点炮' }}
                                </div>
                            </div>

                            <div class="p-4 md:p-6 space-y-3 overflow-y-auto custom-scrollbar">
                                <!-- 胡牌牌面显示 -->
                                <div class="bg-white/90 rounded-xl p-4 shadow-inner">
                                    <div class="text-xs text-gray-500 mb-2 flex items-center justify-between">
                                        ...
                                        <span class="text-red-500 font-bold">胡奖张：<span>{{
                                    ...
                                    <div class="flex flex-wrap gap-y-2 items-end">
                                        ...
                                        <div v-for="(group, idx) in mahjongStore.gameState.roundResult.winnerExposed"
                                            ...
                                            <div v-for="(t, ti) in group.tiles" :key="'ti-' + ti"
                                                class="mahjong-tile-small !bg-white" :class="getTileColorClass(t)">
                                                <span>{{ getTileEmoji(t) }}</span>
                                            </div>
                                        </div>
                                        <!-- 剩余手牌 -->
                                        <div class="flex gap-0.5">
                                            <div v-for="(t, i) in mahjongStore.gameState.roundResult.winnerHand"
                                                :key="'hand-' + i" class="mahjong-tile-small !bg-white"
                                        ...
                                        <div v-if="mahjongStore.gameState.roundResult.winningTile"
                                            class="mahjong-tile-small !w-[32px] !h-[46px] !text-[28px] bg-yellow-50 border-2 border-yellow-400 ml-2 shadow-[0_0_15px_rgba(251,191,36,0.8)] flex items-center justify-center animate-pulse"
                                ...
                                <!-- 番数信息 -->
                                <div class="bg-white/80 rounded-xl p-3 text-center border border-amber-200">
                                    <div class="text-[10px] text-gray-500 mb-0.5">获胜牌型</div>
                                    <div class="text-xl font-bold text-red-600 leading-tight">
                                        {{ mahjongStore.gameState.roundResult.type }}
                                    </div>
                                    <div class="text-2xl font-black text-orange-500 font-serif">
                                ...
                                <div class="space-y-2">
                                    <div v-for="change in mahjongStore.gameState.roundResult.changes" :key="change.name"
                                        class="flex justify-between items-center bg-white/60 rounded-lg px-4 py-2 border-2"
                                        :class="change.isPao || (!mahjongStore.gameState.roundResult.isZiMo && change.amount < 0 && mahjongStore.gameState.roundResult.type !== '流局') ? 'border-orange-500 bg-orange-50/50 ring-4 ring-orange-500/20' : 'border-transparent'">
                                        <div class="flex flex-col">
                                            <span class="font-black text-lg flex items-center gap-2">
                                                {{ change.name }}
                                                <div class="flex gap-1">
                                                    <span
                                                        class="text-[10px] px-1.5 py-0.5 bg-red-600 text-white rounded-sm font-black"
                                                        v-if="change.isWinner">赢家</span>
                                                    <span
                                                        class="text-[10px] px-1.5 py-0.5 bg-gray-700 text-gray-100 rounded-sm font-bold"
                                                        v-else>{{ getPlayerRelation(change.idx) }}</span>
                                                    <span
                                                        class="text-[10px] px-1.5 py-0.5 bg-orange-500 text-white rounded-sm font-black animate-pulse"
                                                        v-if="change.isPao || (!mahjongStore.gameState.roundResult.isZiMo && change.amount < 0 && mahjongStore.gameState.roundResult.type !== '流局')">点炮</span>
                                                </div>
                                            </span>
                                        </div>
                                ...
                                <div class="mt-4 pt-4 border-t border-amber-200">
                                    <div class="flex gap-3">
                                        <button @click.stop="showContactPicker = true"
                                            class="flex-1 bg-white border-2 border-amber-300 text-amber-600 py-4 rounded-2xl font-black text-lg shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
                                            <i class="fa-solid fa-share-nodes"></i> 分享
                                        </button>
                                        <button @click.stop="handleRoundEnd"
                                            class="flex-[2] bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white py-4 rounded-2xl font-black text-xl shadow-xl hover:scale-[1.02] active:scale-95 transition-all">
                                            <i class="fa-solid fa-play mr-2"></i> 再来一局
                                        </button>
                                    </div>
                                    <div class="text-center mt-3 text-[10px] text-gray-400 font-medium">点击任意空白处可先看摊牌
                                    </div>
                                </div>
                            </div>
                        </div>
...
// 检查是否有任何操作可以执行
// 检查是否有任何操作可以执行 (增加了听牌预览按钮)
const canAction = computed(() => canChi.value || canPeng.value || canGang.value || canHu.value || canTing.value)
...
const speak = (type, playerIndex = null) => {
    ...
    const text = tileNames[type] || (type === 'chi' ? '吃' : type === 'peng' ? '碰' : type === 'gang' ? '杠' : (type === 'hu' ?
        '胡' : '过'))
