    ...
    try {
        ...
        if (response.ok) {
            ...
            if (response.status === 404) {
                chatStore.triggerToast('拉取失败: 该API服务不支持获取模型列表（可手动输入模型名称）', 'error')
            } else {
                chatStore.triggerToast('拉取失败: ' + errorMsg + '（可手动输入模型名称）', 'error')
            }
            showManualInput.value = true
        }
    } catch (e) {
        chatStore.triggerToast('拉取失败: ' + e.message + '（可手动输入模型名称）', 'error')
        showManualInput.value = true
    }
}
...
const confirmManualModel = () => {
    if (manualModelInput.value.trim()) {
        formData.value.model = manualModelInput.value.trim()
        chatStore.triggerToast('已手动设置模型: ' + formData.value.model, 'success')
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div class="glass-panel p-5 rounded-[20px] space-y-4">
            ...
            <div>
                ...
                <!-- 手动输入模型名称 - 拉取失败时显示 -->
                <div v-if="showManualInput" class="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                    <p class="text-sm text-amber-700 mb-2">无法自动获取模型列表，请手动输入模型名称：</p>
                    <div class="flex gap-2">
                        <input 
                            v-model="manualModelInput" 
                            type="text" 
                            placeholder="例如：gemini-2.5-flash-minimal"
                            class="px-3 py-2 bg-white border border-amber-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition flex-1 text-sm"
                        >
                        <button 
                            @click="confirmManualModel"
                            class="bg-amber-500 text-white px-3 py-2 rounded-lg hover:bg-amber-600 transition text-sm"
                        >
                            确认
                        </button>
                    </div>
