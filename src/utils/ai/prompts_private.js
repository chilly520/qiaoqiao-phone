...
export function PRIVATE_PROMPT_TEMPLATE(char, user, stickers = [], worldInfo = '', memoryText = '', patSettings = {}, locationContext = '', momentsText = '', bio = {}, linkedGroupMemory = '', contactList = '', calendarContext = '', phoneContext = '', drawingConfig = null) {
    ...
    let drawInstruction = '';
    if (isVolcEngine) {
        // v1.10.122: 火山引擎 Seedream 是中文模型,改为写中文提示词(英文提示词效果差)
        // 同时明确主体规则:提示词里必须明确写出人物主体(我/你/角色名/路人甲),
        // 否则系统无法判断该参考哪张形象图,会走纯文生图不参考形象图
        const volcStyle = drawCfg.globalImageStyle === 'anime' ? '动漫插画风格' : '真实照片风格';
        let drawHint = `    - **发送图片/DRAW**: \`[DRAW: 详细的中文图片提示词]\` (示例：\`[DRAW: 一个穿红裙的女孩在樱花公园里散步，${volcStyle}，自然光线]\`)。
      - **火山引擎是中文生图模型,提示词必须用中文写**,描述要具体(人物外貌、服装、场景、动作、光线、风格)。不要写英文,不要用 Stable Diffusion 的 (keyword:1.5) 权重语法,火山引擎不支持。`;
        if (volcEnabled) {
            if (hasBothAppearance) {
                drawHint += `\n      - **智能形象参考(已开启)**：你已上传你的形象图和${charName}的形象图，系统会根据提示词里的人物主体自动参考对应形象图：
        * 提示词主体是"我"(如"我在拍照"/"我的自拍")→ 参考你的形象图
        * 提示词主体是"你/${charName}"(如"你在公园"/"${charName}的自拍")→ 参考${charName}的形象图
        * 提示词同时有"我和你/我们一起/合照/合影"→ **同时参考两张图**生成你俩的合照(强烈建议多生成合照！)
        * 提示词主体是第三方(如"一个男人"/"一个女孩"/"路人")且没有"我/你"→ **纯文生图,不参考任何形象图**
        * 提示词是"风景/美食/物品/动物"等不涉及人物→ 纯文生图，不参考形象图
      - **⚠️主体明确规则**:生成人物图时,提示词里必须明确写出主体是谁(我/你/${charName}/某个路人),系统靠这个判断参考哪张形象图。严禁写"一个男人给我送礼"这种含"我"但主体是别人的歧义句——要写"一个送礼的男人"或"${charName}送我礼物,我和${charName}在一起"`;
            } else if (hasCharAppearance) {
                drawHint += `\n      - **形象参考(已开启)**：已设置${charName}的形象图。提示词主体明确是"你/${charName}/你的照片/自拍"时,系统会自动参考该形象图。主体是"我"或第三方时不参考。`;
            } else if (hasUserAppearance) {
                drawHint += `\n      - **形象参考(已开启)**：已上传你的形象图。提示词主体明确是"我/我的照片/自拍"时系统会参考你的形象图。主体是第三方(如"一个男人")时不参考。`;
            } else {
