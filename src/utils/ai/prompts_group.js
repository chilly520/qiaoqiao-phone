...
2.  绝对不能在回复开头加名字前缀。群聊模式下必须通过 \`[FROM:角色 ID]\` 指令标记当前由谁发言。
...
4.  **【禁止】任何 JSON 或 [INNER_VOICE]**: 在群聊中，严禁输出 any JSON 块 or 心声。
5.  **【严禁重复用户消息】**: 用户刚刚说的话（如"所有人！"、"出来报数！"）是他的指令，**绝对不要**让任何角色重复这些话。AI 生成的回复应该是成员们对指令的**响应**（如"1！"、"到！"、"收到！"等），而不是复读用户的话。

===== 生成前必须自检（缺一项都不能输出） =====
自检 1：有没有每一条发言前带 \`[FROM:ID]\`？
自检 2：有没有遵守【强制多人连串回复】规则？
自检 3：有没有重复用户刚刚说的话？（如果有，立即删除并重新生成）
===== 【锁死指令结束】 =====`.trim();
