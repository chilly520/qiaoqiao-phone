...
// Force refresh: 2026-03-07-23-46
...
export const useChatStore = defineStore('chat', () => {
    ...
    // MODULE EXTRACTS
    const { _splitRedPacket, claimRedPacket, claimTransfer, claimGift, claimFamilyCard, hasUnclaimedRP } = setupFinancialLogic(chats, addMessage, saveChats, playSound)
    ...
    async function addMessage(chatId, msg) {
        ...
        if (newMsg.type === 'text') {
            // 处理content为数组的情况
            let contentString = newMsg.content;
            if (Array.isArray(contentString)) {
                // 找到type为text的项
                const textItem = contentString.find(item => item.type === 'text');
                if (textItem) {
                    contentString = textItem.text;
                } else {
                    contentString = '';
                }
            }
            ...
            if (typeof contentString === 'string') {
                let detectionContent = contentString.replace(/\[INNER_VOICE\][\s\S]*?(?:\[\/INNER_VOICE\]|$)/gi, '').trim();
                // First try strict full-line match
                // 首先尝试匹配带冒号的格式 [标签:内容]
                let tagMatch = detectionContent.match(/^[\[【](发红包|红包|转账|图片|表情包|DRAW|语音|演奏|MUSIC|VIDEO|FILE|LOCATION|FAMILY_CARD|FAMILY_CARD_APPLY|FAMILY_CARD_REJECT|亲属卡|申请亲属卡|拒绝亲属卡|GIFT|礼物)\s*[:：]\s*([^:：\]】]+)(?:\s*[:：]\s*([^:：\]】]*))?(?:\s*[:：]\s*([^:：\]】]*))?(?:\s*[:：]\s*([^:：\]】]*))?(?:\s*[:：]\s*([^\]】]*))?[\]】]$/i);

                // 如果没有匹配到，尝试匹配不带冒号的格式 [标签] 内容
                if (!tagMatch) {
                    tagMatch = detectionContent.match(/^[\[【](申请亲属卡|FAMILY_CARD_APPLY)[\]】]\s*(.*)$/i)
                    console.log('[addMessage] Non-colon format match:', tagMatch);
                }

                // If no full-line match, try inline match (for embedded tags like "1![表情包：xxx]")
                if (!tagMatch) {
                    tagMatch = detectionContent.match(/[\[【](发红包|红包|转账|图片|表情包|DRAW|语音|演奏|MUSIC|VIDEO|FILE|LOCATION|FAMILY_CARD|FAMILY_CARD_APPLY|FAMILY_CARD_REJECT|亲属卡|申请亲属卡|拒绝亲属卡|GIFT|礼物)\s*[:：]\s*([^:：\]】]+)(?:\s*[:：]\s*([^:：\]】]*))?(?:\s*[:：]\s*([^:：\]】]*))?(?:\s*[:：]\s*([^:：\]】]*))?(?:\s*[:：]\s*([^\]】]*))?[\]】]/i)
                }
                ...
                if (tagMatch) {
                    ...
                    if (/^(发红包|红包)$/.test(tagType)) {
                    ...
                    } else if (tagType === 'FAMILY_CARD' || tagType === '亲属卡') {
                        newMsg.type = 'family_card'
                        newMsg.amount = val1 || '5200'
                        newMsg.text = val2 || '亲属卡'
                        newMsg.paymentId = val3 || null
                        console.log('[addMessage] Detected family_card type:', { tagType, val1, val2, val3 })
                    } else if (tagType === 'FAMILY_CARD_APPLY' || tagType === '申请亲属卡') {
                        newMsg.type = 'family_card_apply'
                        // 检查是否是不带冒号的格式 [申请亲属卡] 留言内容
                        if (tagMatch.length === 3) {
                            // 不带冒号的格式：[申请亲属卡] 留言内容
                            newMsg.note = tagMatch[2] || ''
                            newMsg.paymentId = `PAY-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`
                        } else {
                            // 带冒号的格式：[申请亲属卡:留言:ID]
                            newMsg.note = val1 || ''
                        ...
                        console.log('[addMessage] Detected family_card_apply type:', { tagType, note: newMsg.note, paymentId: newMsg.paymentId })
                    } else if (tagType === 'FAMILY_CARD_REJECT' || tagType === '拒绝亲属卡') {
                    ...
                    // 2.1d Robust Family Card Search (anywhere in message)
                    // Match both bracketed and non-bracketed formats
                    let familyCardAnywhere = detectionContent.match(/[\[【](FAMILY_CARD|亲属卡)\s*[:：]\s*([^:：\]】]+)(?:\s*[:：]\s*([^:：\]】]*))?(?:\s*[:：]\s*([^:：\]】]*))?[\]】]/i);

                    // If no bracketed match, try non-bracketed format (allowing newlines and whitespace)
                    if (!familyCardAnywhere) {
                        // More robust pattern that handles spaces and newlines better
                        familyCardAnywhere = detectionContent.match(/(FAMILY_CARD|亲属卡)\s*[:：]\s*([^:：]+?)(?:\s*[:：]\s*([\s\S]+?))?(?:\s*[:：]\s*([\s\S]+?))?(?=\s*$|\s*\n|\s*\r|\s*\[)/i);
                    }

                    if (familyCardAnywhere) {
                        newMsg.type = 'family_card';
                        newMsg.amount = familyCardAnywhere[2].trim();
                        newMsg.text = familyCardAnywhere[3]?.trim() || '亲属卡';
                        newMsg.paymentId = familyCardAnywhere[4]?.trim() || null;

                        // 尝试从申请卡片中找到匹配的paymentId
                        if (!newMsg.paymentId) {
                            // 查找最近的未处理的申请卡片
                            const applyMsg = chat.msgs.find(m =>
                                m.type === 'family_card_apply' &&
                                !m.isClaimed &&
                                m.status !== 'claimed'
                            );
                            if (applyMsg && applyMsg.paymentId) {
                                newMsg.paymentId = applyMsg.paymentId;
                                console.log('[addMessage] Using paymentId from pending application:', newMsg.paymentId);
                            }
                        }

                        // 如果仍然没有paymentId，生成一个新的
                        newMsg.paymentId = newMsg.paymentId || `PAY-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
                        console.log('[addMessage] Detected family_card from AI:', { amount: newMsg.amount, text: newMsg.text, paymentId: newMsg.paymentId });

                        // 查找对应的申请卡片并更新状态
                        if (newMsg.paymentId) {
                            const applyMsg = chat.msgs.find(m => m.type === 'family_card_apply' && m.paymentId === newMsg.paymentId);
                            if (applyMsg) {
                                applyMsg.status = 'claimed';
                                applyMsg.isClaimed = true;
                                applyMsg.claimTime = Date.now();
                                console.log('[addMessage] Updated family_card_apply status to claimed:', newMsg.paymentId);
                            }
                        }
                    }
                    ...
                    const looseMatch = detectionContent.match(/^[\[【](发红包|红包|转账|申请亲属卡|FAMILY_CARD_APPLY)[\]】]\s*(.*)/i);
                    if (looseMatch) {
                        const tagType = looseMatch[1];
                        const rawText = looseMatch[2].trim();

                        if (/^(申请亲属卡|FAMILY_CARD_APPLY)$/i.test(tagType)) {
                            newMsg.type = 'family_card_apply';
                            newMsg.note = rawText || '';
                            newMsg.paymentId = `PAY-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
                            newMsg.content = `[申请亲属卡:${newMsg.note}:${newMsg.paymentId}]`;
                            console.log('[addMessage] Detected family_card_apply via loose parsing:', { note: newMsg.note, paymentId: newMsg.paymentId });
                        } else {
            ...
            if ((newMsg.type === 'redpacket' || newMsg.type === 'transfer' || newMsg.type === 'family_card' || newMsg.type === 'family_card_apply') && !newMsg.paymentId) {
                newMsg.paymentId = `PAY-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`
            }
            ...
            } else if (newMsg.type === 'family_card') {
                newMsg.amount = msg.amount || 0
                newMsg.text = msg.text || '亲属卡'
                newMsg.paymentId = msg.paymentId || null
                newMsg.isClaimed = msg.isClaimed || false
                newMsg.claimTime = msg.claimTime || null
                newMsg.status = msg.status || 'pending'
            } else if (newMsg.type === 'family_card_apply') {
                newMsg.note = msg.note || ''
                newMsg.paymentId = newMsg.paymentId || msg.paymentId || `PAY-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`
                newMsg.status = msg.status || 'pending'
            } else if (newMsg.type === 'family_card_reject') {
                newMsg.note = msg.note || ''
                newMsg.paymentId = msg.paymentId || null
            ...
            if (newMsg.type === 'redpacket' && newMsg.paymentId) {
                ...
                newMsg.content = `[转账:${newMsg.amount}:${newMsg.note}:${newMsg.paymentId}]`
            } else if (newMsg.type === 'family_card_apply' && newMsg.paymentId) {
                newMsg.content = `[申请亲属卡:${newMsg.note}:${newMsg.paymentId}]`
            }
            ...
            if (newMsg.role === 'ai') {
                ...
                const persistentTypes = ['gift', 'gift_claimed', 'sticker', 'image', 'voice', 'music', 'redpacket', 'transfer', 'family_card', 'family_card_apply', 'moment_card', 'dice_result', 'tarot_card', 'html', 'vote'];
            ...
            if ((newMsg.role === 'assistant' || newMsg.role === 'ai') && typeof newMsg.content === 'string') {
                ...
                const nudgeMatch = content.match(/\[NUDGE\s*[:：]?\s*([^\]]*?)\]/i);
                if (nudgeMatch) {
                    const action = nudgeMatch[1].trim() || '拍了拍你';
            ...
            if ((msgContent.includes('[FAMILY_CARD_APPLY:') || msgContent.includes('[申请亲属卡:')) && newMsg.role === 'user') {
                setTimeout(() => addMessage(chatId, { role: 'system', content: `${userName}正在向${charName}申请绑定亲属卡` }), 100)
            }
            if ((msgContent.includes('[FAMILY_CARD:') || msgContent.includes('[亲属卡:')) && !msgContent.includes('APPLY') && !msgContent.includes('REJECT') && !msgContent.includes('申请') && !msgContent.includes('拒绝') && newMsg.role === 'ai') {
                const match = msgContent.match(/\[(?:FAMILY_CARD|亲属卡):(\d+):([^\]]+)\]/)
                setTimeout(() => addMessage(chatId, { role: 'system', content: `${charName}向您发送了亲属卡，点击领取` }), 100)
            }
            if ((msgContent.includes('[FAMILY_CARD_REJECT:') || msgContent.includes('[拒绝亲属卡:')) && newMsg.role === 'ai') {
...
如果是多选：[VOTE: ${latestVote.title} : 1, 2]`
                    ...
                    const familyCardHint = `\n\n【亲属卡功能】
1. 响应申请：如果用户发送 [申请亲属卡:留言:ID]，你可以根据意愿回复 [亲属卡:额度:名称:ID] 赠送亲属卡给用户。注意 ID 必须与用户申请的 ID 完全一致。
2. 赠送亲属卡：你也可以主动赠送亲属卡，格式：[亲属卡:额度:名称:回执ID]。
3. 领取亲属卡：如果用户给你赠送了亲属卡 [亲属卡:额度:名称:ID]，你可以回复 [领取亲属卡:ID] 来领取。请务必使用卡片消息中提供的 ID。`

                    charInfo.description += voteHint + familyCardHint
...
你可以对这首歌发表看法，或者在觉得氛围合适时，使用 <bgm>歌名 - 歌手</bgm> 格式切换下一首符合当前氛围的歌曲。`
                    ...
                    // Extract FAMILY_CARD tags and store them for separate delivery (including Chinese tags)
                    const familyCardRegex = /\[(?:FAMILY_CARD|亲属卡)(?:_APPLY|申请)?[:：][\s\S]*?\]/gi;
                    ...
                    for (const seg of rawSegments) {
                        ...
                        // PROTECT family_card tags from being split
                        const hasFamilyCard = /\[\s*(?:FAMILY_CARD|亲属卡):[^\]]+\]/i.test(seg);
                        const hasSticker = /\[\s*(?:表情包 | 表情 - 包|STICKER|IMAGE):[^\]]+\]/i.test(seg);
                        const hasInnerVoice = /\[\s*INNER[-_ ]?VOICE\s*\]/i.test(seg);

                        // If contains special tags, extract them first before splitting
                        if (hasFamilyCard || hasSticker || hasInnerVoice) {
                            // Extract and preserve special tags as separate segments
                            const specialTagRegex = /\[\s*(?:FAMILY_CARD|亲属卡|FAMILY_CARD_APPLY|申请亲属卡|表情包 | 表情 - 包|STICKER|IMAGE|INNER[-_ ]?VOICE)[:：][\s\S]*?\[\/\s*(?:INNER[-_ ]?VOICE)?\s*\]|\[\s*(?:FAMILY_CARD|亲属卡|FAMILY_CARD_APPLY|申请亲属卡|表情包 | 表情 - 包|STICKER|IMAGE)[:：][^\]]*?\]/gi;
                            let match;
                            let lastIndex = 0;
                            const workContent = seg;

                            while ((match = specialTagRegex.exec(workContent)) !== null) {
                                // Add text before the tag
                                const textBefore = workContent.substring(lastIndex, match.index).trim();
                                if (textBefore) {
                                    expandedSegments.push(textBefore);
                                }
                                // Add the tag itself
                                expandedSegments.push(match[0]);
                                lastIndex = match.index + match[0].length;
                            }

                            // Add remaining text
                            const remaining = workContent.substring(lastIndex).trim();
                            if (remaining) {
                                expandedSegments.push(remaining);
                            }
                        }
                    ...
                    for (const seg of rawSegments) {
                        let content = seg;
                        console.log('[Split V13] Processing segment:', JSON.stringify(content));
                        ...
                        if (placeholderMatch) {
                        ...
                        } else if (content.startsWith('[NUDGE]') || content.startsWith('[NUDGE:')) {
                        ...
                        } else if (/\[(?:FAMILY_CARD|亲属卡):[^\]]+\]/i.test(content) || /\[(?:FAMILY_CARD_APPLY|申请亲属卡):[^\]]+\]/i.test(content)) {
                            console.log('[Split V13] Detected family_card tag:', content);
                            // Family Card / Apply embedded in text (may also contain other tags like sticker)
                            const familyCardRegex = /\[(?:FAMILY_CARD|亲属卡):([^:\]]+):([^:\]]+)(?::([^:\]]+))?\]/gi;
                            const applyRegex = /\[(?:FAMILY_CARD_APPLY|申请亲属卡):([^:\]]+)(?::([^:\]]+))?\]/gi;

                            let match;
                            const parts = [];
                            let lastIndex = 0;

                            // Copy content to work with it
                            let workContent = content;

                            // Process Applying first (or combined)
                            // Actually, it's easier to just use a unified loop or check one by one
                            // Let's use a simpler approach: check startswith if it's the whole segment, 
                            // but the current logic is in a loop.

                            // If the WHOLE content is the tag:
                            const trimmed = content.trim();
                            if (/^\[(?:FAMILY_CARD|亲属卡):([^:\]]+):([^:\]]+)(?::([^:\]]+))?\]$/i.test(trimmed)) {
                                const m = trimmed.match(/^\[(?:FAMILY_CARD|亲属卡):([^:\]]+):([^:\]]+)(?::([^:\]]+))?\]$/i);
                                finalSegments.push({
                                    type: 'family_card',
                            ...
                            } else if (/^\[(?:FAMILY_CARD_APPLY|申请亲属卡):([^:\]]+)(?::([^:\]]+))?\]$/i.test(trimmed)) {
                                const m = trimmed.match(/^\[(?:FAMILY_CARD_APPLY|申请亲属卡):([^:\]]+)(?::([^:\]]+))?\]$/i);
                                finalSegments.push({
                                    type: 'family_card_apply',
                                    note: m[1]?.trim(),
                                    paymentId: m[2]?.trim(),
                                    content: trimmed,
                                    status: 'pending'
                                ...
                                // Mixed text and tags - split them properly
                                console.log('[Split V13] Processing mixed family_card content:', content);

                                // Extract ALL special tags (sticker, family_card, etc.) and text
                                const stickerRegex = /\[\s*(?:表情包 | 表情 - 包|STICKER|IMAGE)\s*[:：][^\]]*?\]/gi;
                                const fcRegex = /\[\s*(?:FAMILY_CARD|亲属卡):([^:\]]+):([^:\]]+)(?::([^:\]]+))?\s*\]/gi;

                                let processedContent = content;
                                let stickerMatch;
                                let fcMatch;

                                // First, extract and add family card
                                fcMatch = processedContent.match(fcRegex);
                                if (fcMatch && fcMatch.length > 0) {
                                    const fcTag = fcMatch[0];
                                    const fcDetails = fcTag.match(/\[\s*(?:FAMILY_CARD|亲属卡):([^:\]]+):([^:\]]+)(?::([^:\]]+))?\s*\]/i);
                                    if (fcDetails) {
                                        const amount = parseFloat(fcDetails[1]) || 0;
                                        const text = fcDetails[2]?.trim();
                                        const paymentId = fcDetails[3]?.trim();

                                        // Add family card segment
                                        finalSegments.push({
                                            type: 'family_card',
                                            amount: amount,
                                            text: text,
                                        ...
                                        // Remove family card tag from content
                                        processedContent = processedContent.replace(fcTag, '');
                                    }
                                }

                                // Then, extract and add sticker
                                stickerMatch = processedContent.match(stickerRegex);
                                if (stickerMatch && stickerMatch.length > 0) {
                                    const stickerTag = stickerMatch[0];
                                    finalSegments.push({
                                        type: 'sticker',
                                        content: stickerTag
                                    });

                                    // Remove sticker tag from content
                                    processedContent = processedContent.replace(stickerTag, '');
                                }

                                // Add remaining text (if any)
                                const remainingText = processedContent.replace(/\[\s*INNER[-_ ]?VOICE\s*\][\s\S]*/gi, '').trim();
                                if (remainingText) {
                    ...
                    // --- 4. Add extracted family card matches ---
                    // Add family card matches that were extracted earlier
                    if (familyCardMatches && familyCardMatches.length > 0) {
                        familyCardMatches.forEach(fcMatch => {
                            const trimmed = fcMatch.trim();
                            if (/^\[(?:FAMILY_CARD|亲属卡):([^:\]]+):([^:\]]+)(?::([^:\]]+))?\]$/i.test(trimmed)) {
                                const m = trimmed.match(/^\[(?:FAMILY_CARD|亲属卡):([^:\]]+):([^:\]]+)(?::([^:\]]+))?\]$/i);
                                finalSegments.push({
                                    type: 'family_card',
            ...
            console.error('[ChatStore ERROR in sendMessageToAI]', e)
            addMessage(chatId, { role: 'system', content: `[系统生成错误] ${e.message}` })
            typingStatus.value[chatId] = false
        } finally {
            // Ensure typing status is off
            if (!chats.value[chatId]?.pendingSegments || chats.value[chatId]?.pendingSegments.length === 0) {
                typingStatus.value[chatId] = false
            }
            isTyping.value = false;
            console.log('[ChatStore] Done with sendMessageToAI for', chatId)
            saveChats()
        }
    }
    ...
    async function consumePendingSegments(chatId) {
        ...
        try {
            ...
            const claimRegex = /\[领取(红包|转账|亲属卡)[:：]\s*(\w+)\]/gi;
            const rejectRegex = /\[(拒收|退回)(红包|转账|亲属卡)[:：]\s*(\w+)\]/gi;
            ...
            while (chat.pendingSegments.length > 0) {
                ...
                try {
                    ...
                    // Detect type from content if segment is a string or type is undefined
                    let detectedType = type;
                    if (!detectedType || typeof segment === 'string') {
                        const segContent = typeof segment === 'string' ? segment : content;
                        if (/^\[\s*(?:FAMILY_CARD|亲属卡)[:：]/i.test(segContent)) {
                            detectedType = 'family_card';
                            console.log('[Pump] Detected family_card from content');
                        } else if (/^\[\s*(?:FAMILY_CARD_APPLY|申请亲属卡)[:：]/i.test(segContent)) {
                            detectedType = 'family_card_apply';
                            console.log('[Pump] Detected family_card_apply from content');
                        } else if (/^\[\s*(?:表情包 | 表情 - 包|STICKER|IMAGE)[:：]/i.test(segContent)) {
                            detectedType = 'sticker';
                            console.log('[Pump] Detected sticker from content');
                        } else if (/^\[\s*(?:演奏|MUSIC)[:：]/i.test(segContent)) {
                            detectedType = 'music';
                        } else if (/^\[\s*NUDGE[:：]?/i.test(segContent)) {
                            detectedType = 'nudge';
                        } else if (/^\[\s*语音消息[:：]?/i.test(segContent)) {
                            detectedType = 'voice';
                        }
                    }
                    ...
                    console.log('[Pump] Processing segment type:', detectedType, 'content:', msgContent?.substring(0, 50));
                    console.log('[Pump] detectedType === family_card?', detectedType === 'family_card');

                    const whiteListTypes = ['card', 'text', 'redpacket', 'transfer', 'gift', 'sticker', 'family_card', 'family_card_apply', 'music', 'nudge', 'voice', 'call', 'video', 'vote', 'create_vote', 'end_vote'];
                    if (whiteListTypes.includes(detectedType)) {
                        // Payment processing...
                        let claimMatch;
                        while ((claimMatch = claimRegex.exec(msgContent)) !== null) {
                            ...
                            if (targetMsg) {
                                ...
                                else if (pType === '转账') claimTransfer(chatId, targetMsg.id, claimantId);
                                else if (pType === '亲属卡') claimFamilyCard(chatId, targetMsg.id, claimantId);
                            }
                        }
                        let rejectMatch;
                        ...
                        msgContent = msgContent.replace(/\[领取(红包|转账|亲属卡):[^\]]+\]/g, '').replace(/\[(退回|拒收|拒收)(红包|转账|亲属卡):[^\]]+\]/g, '').trim();
                        if (!msgContent) {
                            chat.pendingSegments.shift();
                            saveChats();
                            continue;
                        ...
                        if (msgContent) {
                            console.log('[Pump] msgContent:', msgContent);
                            console.log('[Pump] msgContent length:', msgContent.length);
                            console.log('[Pump] msgContent trimmed:', msgContent.trim());
                            // Check if content contains family_card tag and set type accordingly
                            const fcMatch = msgContent.match(/^\[\s*(?:FAMILY_CARD|亲属卡)[:：]([^:\]]+):([^:\]]+)(?::([^:\]]+))?\s*\]$/i);
                            console.log('[Pump] fcMatch:', fcMatch);
                            if (fcMatch) {
                                // This is a family_card message
                                const msgOptions = {
                                    role: 'ai',
                                    type: 'family_card',
                                    amount: parseFloat(fcMatch[1]) || 0,
                                    text: fcMatch[2]?.trim() || '亲属卡',
                                    paymentId: fcMatch[3]?.trim() || null,
                                    content: msgContent,
                                    isClaimed: false,
                                    ...(currentGroupMeta || {})
                                };
                                console.log('[consumePendingSegments] Processing family_card:', msgOptions);
                                const result = await addMessage(chatId, msgOptions);
                                console.log('[consumePendingSegments] family_card addMessage returned:', result);
                            } else if (detectedType === 'family_card') {
                                // Parse family_card content to extract amount, text, paymentId
                                const fcMatch = msgContent.match(/^\[\s*(?:FAMILY_CARD|亲属卡)[:：]([^:\]]+):([^:\]]+)(?::([^:\]]+))?\s*\]$/i);
                                const msgOptions = {
                                    role: 'ai',
                                    type: 'family_card',
                                    content: msgContent,
                                    amount: fcMatch ? parseFloat(fcMatch[1]) || 0 : (segment.amount || 0),
                                    text: fcMatch ? (fcMatch[2]?.trim() || '亲属卡') : (segment.text || '亲属卡'),
                                    paymentId: fcMatch ? (fcMatch[3]?.trim() || null) : (segment.paymentId || null),
                                    isClaimed: false,
                                    ...(currentGroupMeta || {})
                                };
                                console.log('[consumePendingSegments] Processing family_card:', msgOptions);
                                const result = await addMessage(chatId, msgOptions);
                                console.log('[consumePendingSegments] family_card addMessage returned:', result);
                            } else if (detectedType === 'family_card_apply') {
                                // Parse family_card_apply content to extract note, paymentId
                                const fcaMatch = msgContent.match(/^\[\s*(?:FAMILY_CARD_APPLY|申请亲属卡)[:：]([^:\]]+)(?::([^:\]]+))?\s*\]$/i);
                                const msgOptions = {
                                    role: 'ai',
                                    type: 'family_card_apply',
                                    content: msgContent,
                                    note: fcaMatch ? (fcaMatch[1]?.trim() || '') : (segment.note || ''),
                                    paymentId: fcaMatch ? (fcaMatch[2]?.trim() || null) : (segment.paymentId || null),
                                    status: 'pending',
                                    ...(currentGroupMeta || {})
                                };
                                console.log('[consumePendingSegments] Processing family_card_apply:', msgOptions);
                                const result = await addMessage(chatId, msgOptions);
                                console.log('[consumePendingSegments] family_card_apply addMessage returned:', result);
                            } else {
                                // Spread entire segment to preserve metadata (amount, note, count, etc.)
                                // But first check if segment is an object or string
                                const isSegmentObject = typeof segment === 'object' && segment !== null && !Array.isArray(segment);
                                const msgOptions = {
                                    ...(isSegmentObject ? segment : {}), // Only spread if segment is an object
                                    role: 'ai',
                                    type: detectedType === 'text' ? 'text' : detectedType,
                                    content: msgContent,
                                    forceCard: detectedType === 'card',
                                    ...(currentGroupMeta || {})
                                };

                                // Re-detect type for text messages (e.g., [表情包：xxx] after [FROM:xxx] removal)
                                if (detectedType === 'text' && typeof msgContent === 'string') {
    ...
    return {
        ...
        claimRedPacket, claimTransfer, claimGift, claimFamilyCard,
        hasUnclaimedRP,
