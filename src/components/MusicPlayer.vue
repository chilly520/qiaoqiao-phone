...
const floatingPos = ref({ x: window.innerWidth - 180, y: 100 })
...
const startDrag = (e) => {
    ...
    dragOffset.x = clientX - floatingPos.value.x
    dragOffset.y = clientY - floatingPos.value.y
...
const onDrag = (e) => {
    ...
    // Boundary checks
    const x = Math.max(0, Math.min(window.innerWidth - 160, clientX - dragOffset.x))
    const y = Math.max(0, Math.min(window.innerHeight - 80, clientY - dragOffset.y))

    floatingPos.value.x = x
    floatingPos.value.y = y
}
...
<template>
    <div>
        <!-- Full Player -->
        <div v-if="playerVisible && !musicStore.isMinimized" class="music-player active"
            :class="{ 'together-mode': musicStore.isListeningTogether }">
            <!-- Header -->
            ...
            <div v-if="musicStore.isListeningTogether" class="together-avatars">
                <div class="avatar-group">
                    ...
                    <img :src="musicStore.togetherPartner?.avatar || chatData.charAvatar" class="avatar partner-avatar"
            ...
            <div class="disc-section">
                <div class="disc-container">
                    ...
                    <div class="disc" :class="{ spinning: isPlaying }">
                        ...
                        <img :src="currentSong?.cover || 'https://api.dicebear.com/7.x/shapes/svg?seed=' + (currentSong?.song || 'music')"
                            class="album-cover" @error="handleImageError">
            ...
            <!-- Toolbar -->
            <div class="toolbar">
                <button class="toolbar-btn" :class="{ active: musicStore.repeatMode !== 'off' }"
                    @click="musicStore.switchMode('repeat')" title="循环模式">
                    <i class="fa-solid" :class="musicStore.repeatMode === 'one' ? 'fa-repeat-1' : 'fa-repeat'"></i>
                </button>
                <button class="toolbar-btn" @click="showSearchModal = true" title="搜索"><i
        ...
        <div v-if="playerVisible && musicStore.isMinimized && !musicStore.isListeningTogether" class="music-capsule"
            ...
            <div class="capsule-content">
                <div class="capsule-disc" :class="{ spinning: isPlaying }">
                    <img :src="currentSong?.cover || 'https://api.dicebear.com/7.x/shapes/svg?seed=music'"
                        @error="handleImageError">
                </div>
            ...
            <div class="modal">
                ...
                <div class="search-container">
                    <div class="search-input-wrapper">
                        ...
                        <input v-model="searchQuery" @keyup.enter="executeSearch" type="text" placeholder="搜索歌曲、歌手..."
                    ...
                    <div class="source-chips">
                        <div v-for="opt in [{ id: 'all', label: '全站' }, { id: 'netease', label: '网易云' }, { id: 'tencent', label: 'QQ音乐' }]"
                            :key="opt.id" class="source-chip" :class="{ active: searchSource === opt.id }"
                ...
                <div class="import-wrapper-modern">
                    ...
                    <div class="import-input-group">
                        <input v-model="urlImportInput" type="text" placeholder="粘贴 MP3 链接..."
                            class="import-input-field">
                ...
                <div v-else class="search-results">
                    <div v-if="searchResults.length === 0 && !hasSearched"
                        class="text-center text-gray-500 py-10 text-xs">
                        输入关键词开始搜索
                    </div>
        ...
        <div v-if="showPlaylistModal" class="modal-overlay show" @click.self="showPlaylistModal = false">
            <div class="modal">
                ...
                <div class="playlist-items">
                    ...
                    <div v-for="(song, index) in musicStore.playlist" :key="index" class="playlist-item"
                        :class="{ active: index === musicStore.currentIndex }" @click="musicStore.loadSong(index)">
...
.avatar {
    ...
    height: 52px;
    border-radius: 50%;
    border: 1.5px solid rgba(204, 170, 102, 0.7);
...
@keyframes heartPulse {
    ...
    100% {
        transform: scale(1);
        filter: drop-shadow(0 0 1px rgba(255, 82, 82, 0.3));
    }
...
.lyrics-section:hover {
    ...
    border-color: rgba(204, 170, 102, 0.25);
}
...
.lyrics-text.has-lyric {
    text-shadow: 0 0 6px rgba(204, 170, 102, 0.2);
}
...
.progress-fill {
    height: 100%;
    background: linear-gradient(90deg, #d4af37, #f0c75a);
    width: 0%;
...
.search-submit-icon {
    ...
    height: 32px;
    border-radius: 11px;
    background: linear-gradient(135deg, #ccaa66, #d4af37);
    border: none;
    color: #1a1a1a;
    cursor: pointer;
    display: flex;
...
.import-submit-btn:hover {
    background: rgba(204, 170, 102, 0.2);
}
...
@keyframes pulseHeart {
    ...
    50% {
        ...
        opacity: 1;
        text-shadow: 0 0 20px rgba(255, 77, 79, 1);
    }
...
.playlist-item.active .playlist-song {
    color: #f0c75a;
}
